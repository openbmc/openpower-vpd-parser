...
class IbmHandler
{
  ...
  private:
     ...
     *
     * This method is used when file mode VPD is enabled to restrict VPD
     * publishing to only the system inventory path, removing all other
     * inventory paths from the object map.
     *
     * @param[in,out] io_objectMap - Object map to be filtered. On success,
     *                                it will contain only the system path
     * entry. On failure, it will contain the original map
     */
    void filterToSystemPathOnly(types::ObjectMap& io_objectMap) const noexcept;
