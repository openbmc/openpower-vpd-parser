...
class IbmHandler
{
  ...
  private:
     ...
     *                    if redundant collection succeeded or was not
     * attempted. m_primaryErrno   - errno from SystemException on the primary
     * path, 0 if no SystemException was raised. m_redundantErrno - errno from
     * SystemException on the redundant path if redundant collection failed, 0
