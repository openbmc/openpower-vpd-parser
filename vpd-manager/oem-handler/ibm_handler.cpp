    ...
    const std::string& i_fruPath, types::VPDMapVariant& o_parsedSystemVpdMap)
{
    ...
    catch (const std::exception& l_ex)
    {
        ...
        {
            throw EepromException(l_ex.what());
        }
        ...
        try
        {
            ...
            m_logger->logMessage(
                std::format(
                    "System VPD collection failed from primary path [{}], reason: {}. Successfully collected VPD from redundant path [{}].",
                    i_fruPath, l_ex.what(), l_redundantEepromPath),
        ...
        catch (const std::exception& l_redundantEx)
        {
            throw EepromException(std::format(
                "System VPD collection failed from primary path [{}], reason: {}, and also failed from redundant path [{}], reason: {}.",
                i_fruPath, l_ex.what(), l_redundantEepromPath,
                l_redundantEx.what()));
        }
    }
