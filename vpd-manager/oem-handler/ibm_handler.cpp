    ...
    const std::string& i_fruPath, types::VPDMapVariant& o_parsedSystemVpdMap)
{
    ...
    catch (const std::exception& l_ex)
    {
            ...
            i_fruPath != l_redundantEepromPath)
        {
            m_logger->logMessage(
                std::format(
                    "System VPD collection from primary path [{}] failed: {}. Retrying using redundant EEPROM path [{}]",
                    i_fruPath, EventLogger::getErrorMsg(l_ex),
        ...
        else
        {
            throw std::runtime_error(std::format(
                "System VPD collection failed, reason: {}", l_ex.what()));
        }
    }
