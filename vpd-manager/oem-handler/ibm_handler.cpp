...
void IbmHandler::setDeviceTreeAndJson(const std::string& i_fruPath,
    types::VPDMapVariant& o_parsedSystemVpdMap)
{
    ...
    {
        const std::string& l_message{std::format("System VPD collection failed for path[{}], reason{}",
                i_fruPath, l_ex.what())};
        m_logger->logMessage(l_message);

        const std::string& l_redundantEepromPath{REDUNDANT_SYSTEM_VPD_FILE_PATH};
        if(!l_redundantEepromPath.empty() && i_fruPath != l_redundantEepromPath)
        {
            m_logger->logMessage(std::format(
                "Using redundant EEPROM path [{}] to collect system VPD.", l_redundantEepromPath));
            setDeviceTreeAndJson(l_redundantEepromPath, o_parsedSystemVpdMap);
        }
        else
        {
             throw std::runtime_error(
                "System VPD parsing failed. Either file doesn't exist or error occurred while parsing the file.");
        }
    }
...
void IbmHandler::updateExpandedLocationCode()
{
    ...
    for (const auto& l_inventoryPath : l_objects)
    {
        try
        {
            ...
            if (l_inventoryPath.compare(constants::systemInvPath) ==
                constants::VALUE_0)
            {
                l_expandedLocCode =
            ...
            {
                auto l_chassisString = vpdSpecificUtility::getChassisId(
                    l_inventoryPath, l_errCode);
                ...
                l_expandedLocCode = vpdSpecificUtility::expandFcsLocationCode(
        ...
        {
            std::cerr << "Location code expansion failed for path: " +
                             l_inventoryPath + "reason: " + l_ex.what()
                      << std::endl;
            continue;
        }
        ...
        {
            m_logger->logMessage(std::format(
                "Dbus call to publish expanded location code for inventory path {} failed.",
                l_inventoryPath));
        }
    }
}
