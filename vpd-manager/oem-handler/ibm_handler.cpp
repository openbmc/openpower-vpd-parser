...
void IbmHandler::initBackupAndRestore() noexcept
{
    try
    {
        ...
        {
            // Throwing as empty sysconfig JSON is not expected at this point of
            // execution and also not having backup and restore object will
        ...
        uint16_t l_errCode = 0;
        if (jsonUtility::isBackupAndRestoreRequired(m_sysCfgJsonObj, l_errCode))
        {
        ...
        if (l_errCode)
        {
            // Throwing as setting of error code confirms that back up and
            // restore object was not initialized. This will effect system VPD
            // sync.
            throw std::runtime_error(
...
void IbmHandler::performBackupAndRestore(types::VPDMapVariant& io_srcVpdMap)
{
    try
    {
        ...
        {
            throw std::runtime_error(
                "Unable to get back up and restore object.");
        }
    ...
    { // Skipping setting device tree as either devtree info is missing from
        ...
        if (!l_sysVpdObjMap.empty())
        {
            if (isBackupOnCache())
            {
                ...
                {
                    m_logger->logMessage(
                        "Backup and restore was not performed as object creation failed. Proceeding wth publishing of system VPD.");
                }
            }
        }
