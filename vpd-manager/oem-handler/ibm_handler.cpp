...
void IbmHandler::checkAndUpdateBmcPosition(size_t& o_bmcPosition) const noexcept
{
    try
    {
        gpiod::line l_positionGpio =
            gpiod::find_line(constants::prototypeSysPositionGpio);
        if (!l_positionGpio)
        {
            m_logger->logMessage("Could not find gpio line" +
                                 constants::prototypeSysPositionGpio +
        ...
        if (l_presenceLine.get_value() == constants::VALUE_0)
        {
    ...
    {
        m_logger->logMessage("Exception while processing gpio" +
                             constants::prototypeSysPositionGpio +
