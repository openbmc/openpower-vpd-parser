    ...
    const std::string& i_fruPath, types::VPDMapVariant& o_parsedSystemVpdMap)
{
    ...
    // Log a PEL when system VPD is collected from the redundant path
    if (i_fruPath != SYSTEM_VPD_FILE_PATH)
    {
        ...
        m_logger->logMessage(
            ...
            types::PelInfoTuple{
                types::ErrorType::InvalidEeprom, types::SeverityType::Warning,
                0, std::nullopt, std::nullopt, std::nullopt, std::nullopt,
                std::make_tuple(SYSTEM_VPD_FILE_PATH, types::CalloutPriority::High)});
    }
