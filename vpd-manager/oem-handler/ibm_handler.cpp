    ...
    const std::string& i_fruPath, types::VPDMapVariant& o_parsedSystemVpdMap)
{
    ...
    {
        static std::string l_primaryVpdError{};
        ...
        {
            l_primaryVpdError = std::format(
                "System VPD collection from primary path [{}] failed for reason: {} .",
                i_fruPath, l_ex.what());
            ...
            // implemented.
            m_logger->logMessage(
                l_primaryVpdError, PlaceHolder::ASYNC_PEL_WITH_INV_CALLOUT,
                types::PelInfoTuple{
        ...
        else
        {
            ...
            if (l_primaryVpdError.empty())
            {
                l_message = std::format(
            ...
            else
            {
                l_message =
                    std::format(
                        "System VPD collection failed from redundant path {}, reason: {}. ",
                        i_fruPath, l_ex.what()) +
                    l_primaryVpdError;
            }
            ...
            // implemented.
            m_logger->logMessage(
                l_message, PlaceHolder::ASYNC_PEL_WITH_INV_CALLOUT,
                types::PelInfoTuple{
                    EventLogger::getErrorType(l_ex),
                    types::SeverityType::Warning, 0, std::nullopt, std::nullopt,
                    std::nullopt, std::nullopt,
