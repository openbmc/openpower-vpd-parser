...
bool IbmHandler::isSymlinkPresent() noexcept
{
    ...
    if (!std::filesystem::exists(INVENTORY_JSON_SYM_LINK, l_ec))
    {
        ...
        {
            // Critical PEL logged as symlink can't go missing while chassis is
            // on.
        ...
        {
            m_logger->logMessage(
                "Error reading symlink location. Proceeding as not present.");
        }
        return false;
    }
    else
...
void Worker::setJsonSymbolicLink(const std::string& i_systemJson)
{
    ...
    m_logger->logMessage("JSON symlink modified to: " + i_systemJson);
