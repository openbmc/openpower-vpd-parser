    ...
    catch (const std::exception& l_ex)
    {
        ...
        if (i_fruPath == SYSTEM_VPD_FILE_PATH)
        {
            ...
            catch (const std::exception& l_redundantEx)
            {
                ...
                const std::string l_redundantError =
                    std::format("Redundant path [{}] also failed: {}",
...
void IbmHandler::performInitialSetup()
{
    ...
    catch (const std::exception& l_ex)
    {
        ...
        {
            // PEL #1 (primary callout) was already logged in
            // setDeviceTreeAndJson. Log PEL #2 here with redundant path
            // callout.
...
=======
            l_callout = types::DeviceCalloutData{
                ...
                types::CalloutPriority::High};
>>>>>>> 7500cfa (Make use of I2C device callout)
        }
