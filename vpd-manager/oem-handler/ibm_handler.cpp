...
void IbmHandler::performInitialSetup()
{
    try
    {
        ...
        if (isRbmcProtoTypeSystem(l_errCode) && l_errCode == 0)
        {
            ...
            notifyRbmcPosition(l_rbmcPosition);
        }
...
bool IbmHandler::canAccessMotherboardEeprom() const noexcept
{
...
void IbmHandler::notifyRbmcPosition(
    [[maybe_unused]] const size_t i_position) const noexcept
{
