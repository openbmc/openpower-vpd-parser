                           ...
                           const std::string keyword, const Binary value)
{
    try
    {
        ...
        std::unordered_map<std::string, std::vector<std::string>> commonIntRecordsList = {
        ...
        {
            isCI = true;
            vpdFilePath = getSysPathForThisFruType(jsonFile, path, "fruAndModule");
        }
        else
        {
        ...
        // If it is not a CPU fru then go ahead with default vpdFilePath from fruMap
