...
class Logger
{
  ...
  private:
     ...
     */
    Logger(const std::shared_ptr<sdbusplus::asio::connection>& i_asioConnection):m_vpdWriteLogger(nullptr)
    {
    ...
    // Dbus connection
    static inline std::shared_ptr<sdbusplus::asio::connection> m_connection = nullptr;
