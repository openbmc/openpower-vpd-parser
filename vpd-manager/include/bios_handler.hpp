...
class IbmBiosHandler : public BiosHandlerInterface
{
  public:
        ...
        m_manager(i_manager),m_worker(i_manager->getWorkerObj())
    {
       ...
       std::string l_backupAndRestoreCfgFilePath = l_sysCfgJsonObj.value("biosHandlerJsonPath", "");

       if (l_backupAndRestoreCfgFilePath.empty())
       {
        logging::logMessage("Critical: BiosHandlerJsonPath key is missing in config.");
