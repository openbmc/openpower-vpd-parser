...
inline auto map(int128_opt x) -> int128_opt
{
    return x;
}
inline auto map(uint128_opt x) -> uint128_opt
{
    return x;
}
#else
...
inline auto map(int128_opt) -> monostate
{
    return {};
}
inline auto map(uint128_opt) -> monostate
{
    return {};
}
#endif
...
template <int N>
using ubitint = unsigned _BitInt(N);
#else
template <int N>
struct bitint
{};
...
enum
{
    use_utf8 = !FMT_WIN32 || is_utf8_enabled
};
...
class basic_string_view
{
  ...
  public:
    ...
    FMT_CONSTEXPR20 basic_string_view(const Char* s) : data_(s)
    {
        ...
        {
            size_ =
                __builtin_strlen(detail::narrow(s)); // strlen is not costexpr.
            return;
        }
#endif
        size_t len = 0;
...
class basic_specs
{
  ...
  public:
    ...
    {
        return static_cast<arg_id_kind>(
            (data_ & precision_mask) >> precision_shift);
    }
...
class parse_context
{
  private:
    ...
    enum
    {
        use_constexpr_cast = !FMT_GCC_VERSION || FMT_GCC_VERSION >= 1200
    };
  ...
  public:
    ...
    /// Returns an iterator past the end of the format string range being
    /// parsed.
    constexpr auto end() const noexcept -> iterator
    {
        return fmt_.end();
    ...
    /// Reports an error if using the manual argument indexing; otherwise
    /// returns the next argument index and switches to the automatic indexing.
                                       ...
                                       basic_string_view<Char> arg_name)
{
    ...
    {
        if (named_args[i].name == arg_name)
            report_error("duplicate named arg");
    }
}
...
struct type_mapper
{
    ...
    static auto map(T)
        -> conditional_t<std::is_same<T, char>::value ||
...
class compile_parse_context : public parse_context<Char>
{
  ...
  public:
    FMT_CONSTEXPR explicit compile_parse_context(
    ...
    else
    {
        if (*begin == '{')
        {
            ...
            if (begin != end)
            {
                ...
                {
                    begin = parse_arg_id(
                        begin, end, dynamic_spec_handler<Char>{ctx, ref, kind});
                }
            }
            if (begin != end && *begin == '}')
                return {++begin, kind};
    ...
    -> const Char*
{
    ...
    for (;;)
    {
        switch (c)
        {
            ...
            case '0':
                ...
                {
                    // Ignore 0 if align is specified for compatibility with
                    // std::format.
    ...
    case '6': case '7': case '8': case '9': case '{':
            ...
            case '.':
                ...
                begin = parse_precision(begin, end, specs, specs.precision_ref,
                                        ctx);
                break;
            case 'L':
            ...
            case 'p':
                return parse_presentation_type(pres::pointer,
                                               pointer_set | cstring_set);
            case '?':
                return parse_presentation_type(
            ...
            default:
            {
                ...
                specs.set_fill(basic_string_view<Char>(
                    begin, to_unsigned(fill_end - begin)));
...
FMT_CONSTEXPR auto invoke_parse(parse_context<Char>& ctx) -> const Char*
{
    ...
    if (!formattable)
        return ctx.begin(); // Error is reported in the value ctor.
...
class format_string_checker
{
  ...
  public:
    ...
    FMT_CONSTEXPR explicit format_string_checker(basic_string_view<Char> fmt,
                                                 arg_pack<T...>) :
        types_{mapped_type_constant<T, Char>::value...}, named_args_{},
        context_(fmt, NUM_ARGS, types_), parse_funcs_{&invoke_parse<T, Char>...}
    {
                                       ...
                                       const Char* end) -> const Char*
    {
        ...
        // If id is out of range, it means we do not know the type and cannot
        // parse the format at compile time. Instead, skip over content until we
        // finish the format spec, accounting for any nested replacements.
...
class buffer
{
  ...
  public:
    ...
    // Tries increasing the buffer capacity to `new_capacity`. It can increase
    // the capacity by a smaller amount than requested but guarantees there is
    // space for at least one additional element either by increasing the
    // capacity or by flushing the buffer if it is full.
...
class iterator_buffer : public Traits, public buffer<T>
{
  ...
  public:
    ...
    iterator_buffer(iterator_buffer&& other) noexcept :
        Traits(other), buffer<T>(grow, data_, 0, buffer_size), out_(other.out_)
    {}
    ~iterator_buffer()
    {
    ...
    public buffer<T>
{
  ...
  public:
    ...
    iterator_buffer(iterator_buffer&& other) noexcept :
        ...
        buffer<T>(static_cast<iterator_buffer&&>(other)), out_(other.out_)
    {
        if (this->data() != out_)
        {
            this->set(data_, buffer_size);
...
class counting_buffer : public buffer<T>
{
  private:
    enum
    {
...
struct custom_value
{
    ...
    void* value;
    void (*format)(void* arg, parse_context<char_type>& parse_ctx,
                   Context& ctx);
};
...
class value
{
  public:
    ...
    value(const T&)
    {
        // Formatting of arbitrary pointers is disallowed. If you want to format
        // a pointer cast it to `void*` or `const void*`. In particular, this
        // forbids formatting of `[const] volatile char*` printed as bool by
        // iostreams.
    ...
    FMT_ALWAYS_INLINE value(const named_arg_info<char_type>* args,
                            size_t size) : named_args{args, size}
    {}
  ...
  private:
    ...
    FMT_CONSTEXPR value(T& x, custom_tag)
    {
        ...
        custom.format =
            format_custom<value_type, formatter<value_type, char_type>>;
    }
                              ...
                              Context& ctx)
    {
        ...
        // format must be const for compatibility with std::format and
        // compilation.
        const auto& cf = f;
...
{
    // +1 to workaround a bug in gcc 7.5 that causes duplicated-branches
    // warning.
    using type =
...
struct native_formatter
{
  ...
  public:
    ...
    {
        specs_.set_type(set ? presentation_type::debug
...
FMT_CONSTEXPR void parse_context<Char>::check_dynamic_spec(int arg_id)
{
    ...
    if (detail::is_constant_evaluated() && use_constexpr_cast)
        static_cast<compile_parse_context<Char>*>(this)->check_dynamic_spec(
            arg_id);
}
...
class basic_format_args
{
  private:
    ...
    // argument types are passed in the descriptor. This reduces binary code
    // size per formatting function call.
    unsigned long long desc_;
    union
    {
        // If is_packed() returns true then argument values are stored in
        // values_; otherwise they are stored in args_. This is done to improve
        // cache locality and reduce compiled code size since storing larger
        // objects may require more code (at least on x86-64) even if the same
        // amount of data is actually copied to stack. It saves ~10% on the
        // bloat test.
