                        ...
                        int& ec) -> To
{
    ...
    // maybe we should also check if from is denormal, and decide what to do
    // about it.
...
struct null
{};
inline auto localtime_r FMT_NOMACRO(...) -> null<>
{
    return null<>();
}
inline auto localtime_s(...) -> null<>
{
    return null<>();
}
...
class formatbuf : public StreamBuf
{
  ...
  protected:
    // The put area is always empty. This makes the implementation simpler and
    // has the advantage that the streambuf and the buffer are always in sync
    // and sputc never writes into uninitialized memory. A disadvantage is that
    // each call to sputc always results in a (virtual) call to overflow. There
    // is no disadvantage here for sputn since this always results in a call to
    // xsputn.
    ...
    -> OutputIt
{
    ...
    (defined(__GLIBCXX__) &&                                                   \
     (!defined(_GLIBCXX_USE_DUAL_ABI) || _GLIBCXX_USE_DUAL_ABI == 0))
        // The _GLIBCXX_USE_DUAL_ABI macro is always defined in libstdc++ from
        // gcc-5 and newer.
        ...
        auto u = to_utf8<code_unit,
                         basic_memory_buffer<char, unit_t::max_size * 4>>();
...
using is_similar_arithmetic_type = bool_constant<
...
auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To
{
    ...
    {
        if (count > max_value<common_rep>() / factor::num)
            throw_duration_error();
...
FMT_DEPRECATED inline auto localtime(std::time_t time) -> std::tm
{
    struct dispatcher
    {
        ...
        inline auto handle(std::tm* tm) -> bool
        {
            return tm != nullptr;
        }
        ...
        inline auto fallback(int res) -> bool
        {
            return res == 0;
        }
...
#if FMT_USE_LOCAL_TIME
template <typename Duration>
FMT_DEPRECATED auto localtime(std::chrono::local_time<Duration> time) -> std::tm
{
...
FMT_CONSTEXPR inline auto get_units() -> const char*
{
    ...
    if (std::is_same<Period, std::ratio<3600>>::value)
        return "h";
    if (std::is_same<Period, std::ratio<86400>>::value)
        return "d";
    return nullptr;
}
                                       ...
                                       Handler&& handler) -> const Char*
{
    ...
    while (ptr != end)
    {
        ...
        switch (c)
        {
            ...
            // Year:
            case 'Y':
                handler.on_year(numeric_system::standard, pad);
                break;
            case 'y':
            ...
            case 'B':
                ...
                break;
            case 'm':
                handler.on_dec_month(numeric_system::standard, pad);
                break;
            ...
            case 'E':
            {
                ...
                {
                    case 'Y':
                        handler.on_year(numeric_system::alternative, pad);
                        break;
                    case 'y':
            ...
            case 'O':
                ...
                switch (c)
                {
                    case 'y':
                        ...
                        break;
                    case 'm':
                        handler.on_dec_month(numeric_system::alternative, pad);
                        break;
                    case 'U':
                        handler.on_dec0_week_of_year(
                            numeric_system::alternative, pad);
                        break;
                    case 'W':
...
class tm_format_checker : public null_chrono_spec_handler<tm_format_checker>
{
  ...
  public:
    ...
    {
        if (!has_timezone_)
            FMT_THROW(format_error("no timezone"));
    }
    FMT_CONSTEXPR void on_tz_name()
    {
        if (!has_timezone_)
...
class tm_writer
{
  private:
    ...
    // concatenated with %y yields the same output as %Y, and that %Y contains
    // at least 4 characters, with more only if necessary.
  ...
  public:
    ...
    void on_dec0_week_of_year(numeric_system ns, pad_type pad)
    {
        if (is_classic_ || ns == numeric_system::standard)
            return write2(
                (tm_yday() + days_per_week - tm_wday()) / days_per_week, pad);
        format_localized('U', 'O');
    }
...
struct duration_formatter
{
        ...
        out(o), val(static_cast<rep>(d.count())), locale(loc), negative(false)
    {
        ...
        s = detail::duration_cast<seconds>(
    ...
    auto days() const -> Rep
    {
        return static_cast<Rep>(s.count() / 86400);
    }
    auto hour() const -> Rep
    {
        ...
        if (ns == numeric_system::standard)
        {
            if (std::is_floating_point<rep>::value)
            {
                ...
                write_floating_seconds(
                    buf, std::chrono::duration<rep, Period>(val), precision);
                if (negative)
                    *out++ = '-';
                if (buf.size() < 2 || buf[1] == '.')
    ...
    void on_duration_unit()
    {
        out = format_duration_unit<Char, Period>(out);
    }
};
...
struct formatter<weekday, Char> : private formatter<std::tm, Char>
{
  ...
  public:
    ...
    auto format(weekday wd, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        auto w =
            detail::tm_writer<decltype(ctx.out()), Char>(loc, ctx.out(), time);
        w.on_abbr_weekday();
        return w.out();
    }
};
...
struct formatter<std::chrono::duration<Rep, Period>, Char>
{
  ...
  public:
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char*
    {
        ...
        if (*it == '.')
        {
            checker.has_precision_integral =
        ...
        -> decltype(ctx.out())
    {
        ...
        // As a possible future optimization, we could avoid extra copying if
        ...
        {
            out =
                detail::format_duration_value<Char>(out, d.count(), precision);
            detail::format_duration_unit<Char, Period>(out);
        }
        else
        {
            auto f = detail::duration_formatter<Char, Rep, Period>(
                out, d, ctx.locale());
...
struct formatter<std::tm, Char>
{
  ...
  protected:
        ...
        -> const Char*
    {
        ...
        end = detail::parse_chrono_format(
            it, end, detail::tm_format_checker(has_timezone));
...
struct formatter<sys_time<Duration>, Char> : private formatter<std::tm, Char>
{
        ...
        -> decltype(ctx.out())
    {
        ...
        {
            auto second =
                detail::duration_cast<Duration>(std::chrono::seconds(1));
            if (tm.tm_sec != 0)
            {
                --tm.tm_sec;
            }
...
struct formatter<utc_time<Duration>, Char> : formatter<sys_time<Duration>, Char>
{
    template <typename FormatContext>
...
template <typename Duration, typename Char>
struct formatter<local_time<Duration>, Char> : private formatter<std::tm, Char>
{
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char*
    {
        ...
        -> decltype(ctx.out())
    {
        ...
        auto subsecs = detail::duration_cast<Duration>(
            time_since_epoch - seconds_since_epoch);
