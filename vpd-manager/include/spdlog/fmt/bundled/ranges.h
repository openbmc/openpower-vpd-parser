...
struct has_const_begin_end<
    T, void_t<decltype(*detail::range_begin(
              ...
              decltype(detail::range_end(
                  std::declval<const remove_cvref_t<T>&>()))>> : std::true_type
{};
...
struct format_tuple_element
{
    ...
    {
        if (i > 0)
            ctx.advance_to(detail::copy<char_type>(separator, ctx.out()));
...
struct range_format_kind :
    conditional_t<is_range<T, Char>::value, detail::range_format_kind_<T>,
  ...
  public:
    ...
    FMT_CONSTEXPR formatter()
    {
        if (detail::const_check(
                range_format_kind<R, Char>::value != range_format::set))
            return;
        ...
        detail::is_formattable_delayed<R, Char>>::value>>
{
  ...
  public:
    ...
    auto format(map_type& map, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        if (!no_delimiters_)
            out = detail::copy<Char>(open, out);
        int i = 0;
        basic_string_view<Char> sep = detail::string_literal<Char, ',', ' '>{};
        for (auto&& value : map)
        {
            if (i > 0)
                out = detail::copy<Char>(sep, out);
            ctx.advance_to(out);
            detail::for_each2(
                formatters_, value,
        ...
        if (!no_delimiters_)
            out = detail::copy<Char>(close, out);
        return out;
    }
};
  ...
  public:
        ...
        -> decltype(ctx.out())
    {
        ...
        out = underlying_.format(
            string_type{detail::range_begin(range), detail::range_end(range)},
            ctx);
        if (detail::const_check(range_format_kind<R, Char>::value ==
                                range_format::debug_string))
            *out++ = '"';
  ...
  private:
        ...
        -> const Char*
    {
        auto end = ctx.begin();
            ...
            std::get<std::tuple_size<Tuple>::value - N>(formatters_).parse(ctx);
        if (N > 1)
        {
    ...
    auto do_format(const tuple_join_view<Char, Tuple>& value,
                   FormatContext& ctx, std::integral_constant<size_t, N>) const
        -> typename FormatContext::iterator
    {
        using std::get;
        auto out =
            std::get<std::tuple_size<Tuple>::value - N>(formatters_)
                .format(get<std::tuple_size<Tuple>::value - N>(value.tuple),
                        ctx);
        if (N <= 1)
            return out;
