                        ...
                        const std::basic_string<PathChar>& native)
{
                  ...
                  std::is_same_v<PathChar, wchar_t>)
    {
        ...
        bool valid =
            to_utf8<wchar_t>::convert(quoted, {buf.data(), buf.size()});
    ...
    {
        write_escaped_string<Char>(std::back_inserter(quoted),
                                   p.string<Char>());
    }
}
...
struct formatter<std::filesystem::path, Char>
{
  ...
  public:
    ...
    auto format(const std::filesystem::path& p, FormatContext& ctx) const
    {
        ...
        return detail::write(
            ctx.out(), basic_string_view<Char>(quoted.data(), quoted.size()),
            specs);
    }
};
...
class path : public std::filesystem::path
{
  public:
    ...
    auto system_string() const -> std::string
    {
        return string();
    }
...
template <typename T>
struct has_flip<T, void_t<decltype(std::declval<T>().flip())>> : std::true_type
{};
  ...
  public:
        ...
        -> decltype(ctx.out())
    {
        ...
        if (specs.dynamic())
        {
            detail::handle_dynamic_spec(specs.dynamic_width(), specs.width,
                                        specs.width_ref, ctx);
            detail::handle_dynamic_spec(specs.dynamic_precision(),
                                        specs.precision, specs.precision_ref,
                                        ctx);
        }
