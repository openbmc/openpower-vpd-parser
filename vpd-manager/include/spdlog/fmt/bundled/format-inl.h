...
#if FMT_USE_LOCALE
    if (locale_)
        return *static_cast<const locale*>(locale_);
#endif
    return locale();
}
...
format_facet<Locale>::format_facet(Locale& loc)
{
    ...
    if (!grouping_.empty())
        separator_ = std::string(1, np.thousands_sep());
}
...
{
    return val.visit(detail::loc_writer<>{out, specs, separator_, grouping_,
                                          decimal_point_});
}
#endif
...
auto check_divisibility_and_divide_by_pow10(uint32_t& n) noexcept -> bool
{
    ...
    // Multiplication" by Granlund and Montgomery (1994). magic_number (m) is
    // set to ceil(2^k/d) for large enough k. The idea for item 2 originates
    // from Schubfach.
...
struct cache_accessor<float>
{
    ...
    {
        FMT_ASSERT(k >= float_info<float>::min_k &&
                       k <= float_info<float>::max_k,
                   "k is out of range");
...
struct cache_accessor<double>
{
    ...
    static auto get_cached_power(int k) noexcept -> uint128_fallback
    {
        ...
        int alpha = floor_log2_pow10(kb + offset) - floor_log2_pow10(kb) -
                    offset;
...
FMT_INLINE decimal_fp<T> shorter_interval_case(int exponent) noexcept
{
    ...
    auto xi =
        cache_accessor<T>::compute_left_endpoint_for_shorter_interval_case(
            cache, beta);
    auto zi =
        cache_accessor<T>::compute_right_endpoint_for_shorter_interval_case(
            cache, beta);
...
auto to_decimal(T x) noexcept -> decimal_fp<T>
{
    ...
    { // Check if normal.
        ...
        // regular. However, it can be shown that the end-results are anyway
        // same.
        if (significand == 0)
...
template <typename F>
class glibc_file : public file_base<F>
{
  private:
    enum
    {
  ...
  public:
    ...
    void init_buffer()
    {
        if (this->file_->_IO_write_ptr < this->file_->_IO_write_end)
            return;
        // Force buffer initialization by placing and removing a char in a
        // buffer.
    ...
    {
        if ((this->file_->_flags & line_buffered) == 0)
            return false;
    ...
    void flush()
    {
        fflush_unlocked(this->file_);
    }
};

// A FILE wrapper for Apple's libc.
template <typename F>
class apple_file : public file_base<F>
{
  private:
    enum
    {
...
template <typename F>
class fallback_file : public file_base<F>
{
  private:
    ...
    public buffer<char>
{
  private:
    ...
    static void grow(buffer<char>& base, size_t)
    {
        ...
        if (self.file_.get_write_buffer().size == 0)
            self.file_.flush();
