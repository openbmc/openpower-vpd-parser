...
class tcp_client
{
  ...
  public:
    ...
    int connect_socket_with_timeout(int sockfd, const struct sockaddr* addr,
                                    socklen_t addrlen, const timeval& tv)
    {
    ...
    void connect(const std::string& host, int port, int timeout_ms = 0)
    {
        ...
        auto rv = ::getaddrinfo(host.c_str(), port_str.c_str(), &hints,
                                &addrinfo_result);
        if (rv != 0)
        {
            throw_spdlog_ex(
        ...
        for (auto* rp = addrinfo_result; rp != nullptr; rp = rp->ai_next)
        {
            ...
            const int flags = 0;
#endif
            socket_ = ::socket(rp->ai_family, rp->ai_socktype | flags,
                               rp->ai_protocol);
            if (socket_ == -1)
            {
                last_errno = errno;
            ...
            if (connect_socket_with_timeout(socket_, rp->ai_addr,
                                            rp->ai_addrlen, tv) == 0)
            {
                last_errno = 0;
                break;
        ...
        if (timeout_ms > 0)
        {
            ...
            ::setsockopt(socket_, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv,
                         sizeof(tv));
            ::setsockopt(socket_, SOL_SOCKET, SO_SNDTIMEO, (const char*)&tv,
                         sizeof(tv));
        }
        ...
        ::setsockopt(socket_, IPPROTO_TCP, TCP_NODELAY,
                     reinterpret_cast<char*>(&enable_flag),
                     sizeof(enable_flag));
...
#if !defined(SO_NOSIGPIPE) && !defined(MSG_NOSIGNAL)
#error                                                                         \
    "tcp_sink would raise SIGPIPE since neither SO_NOSIGPIPE nor MSG_NOSIGNAL are available"
#endif
    }
    ...
    void send(const char* data, size_t n_bytes)
    {
        ...
        while (bytes_sent < n_bytes)
        {
            ...
            if (write_result ==
                0) // (probably should not happen but in any case..)
            {
                break;
            }
