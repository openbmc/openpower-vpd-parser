...
#elif __APPLE__
    uint64_t tid;
// There is no pthread_threadid_np prior to Mac OS X 10.6, and it is not
// supported on any PPC, including 10.6.8 Rosetta. __POWERPC__ is Apple-specific
// define encompassing ppc and ppc64.
...
SPDLOG_INLINE void wstr_to_utf8buf(wstring_view_t wstr, memory_buf_t& target)
{
    ...
    if (result_size > 0)
    {
        ...
        result_size =
            ::WideCharToMultiByte(CP_UTF8, 0, wstr.data(), wstr_size,
...
SPDLOG_INLINE void utf8_to_wstrbuf(string_view_t str, wmemory_buf_t& target)
{
    ...
    {
        target.resize(result_size);
        result_size = ::MultiByteToWideChar(CP_UTF8, 0, str.data(), str_size,
...
SPDLOG_INLINE bool fsync(FILE* fp)
{
    ...
    return FlushFileBuffers(
               reinterpret_cast<HANDLE>(_get_osfhandle(_fileno(fp)))) != 0;
#else
