...
class scoped_padder
{
  public:
    scoped_padder(size_t wrapped_size, const padding_info& padinfo,
                  memory_buf_t& dest) : padinfo_(padinfo), dest_(dest)
    {
        remaining_pad_ =
...
class elapsed_formatter final : public flag_formatter
{
  public:
    ...
    void format(const details::log_msg& msg, const std::tm&,
                memory_buf_t& dest) override
    {
        auto delta = (std::max)(msg.time - last_message_time_,
                                log_clock::duration::zero());
...
// Example: [logger-name] [info] [mdc_key_1:mdc_value_1 mdc_key_2:mdc_value_2]
// some message
#ifndef SPDLOG_NO_TLS
template <typename ScopedPadder>
...
SPDLOG_INLINE std::unique_ptr<formatter> pattern_formatter::clone() const
{
    ...
    auto cloned = details::make_unique<pattern_formatter>(
        pattern_, pattern_time_type_, eol_,
    ...
    char flag, details::padding_info padding)
{
    ...
    switch (flag)
    {
        ...
        case ('u'): // elapsed time since last log message in nanos
            formatters_.push_back(
                details::make_unique<details::elapsed_formatter<
                    Padder, std::chrono::nanoseconds>>(padding));
            break;

        case ('i'): // elapsed time since last log message in micros
            formatters_.push_back(
                details::make_unique<details::elapsed_formatter<
                    Padder, std::chrono::microseconds>>(padding));
            break;

        case ('o'): // elapsed time since last log message in millis
            formatters_.push_back(
                details::make_unique<details::elapsed_formatter<
                    Padder, std::chrono::milliseconds>>(padding));
            break;
        ...
        case ('&'):
            formatters_.push_back(
                details::make_unique<details::mdc_formatter<Padder>>(padding));
            break;
#endif
        ...
        default: // Unknown flag appears as is
            ...
            // fix issue #1617 (prev char was '!' and should have been treated
            // as funcname flag instead of truncating flag)
