...
// If the widget's lifetime can be shorter than the logger's one, you should
// provide some permanent QObject, and then use a standard signal/slot.
//
...
// Color location is determined by the sink log pattern like in the rest of
// spdlog sinks. Colors can be modified if needed using sink->set_color(level,
// qtTextCharFormat). max_lines is the maximum number of lines that the sink
// will hold before removing the oldest lines. By default, only ascii (latin1)
// is supported by this sink. Set is_utf8 to true if utf8 support is needed.
template <typename Mutex>
class qt_color_sink : public base_sink<Mutex>
{
  public:
    qt_color_sink(QTextEdit* qt_text_edit, int max_lines,
                  bool dark_colors = false, bool is_utf8 = false) :
  ...
  protected:
    ...
    {
        invoke_params(int max_lines, QTextEdit* q_text_edit, QString payload,
                      QTextCharFormat default_color,
                      QTextCharFormat level_color, int color_range_start,
                      int color_range_end) :
            max_lines(max_lines), q_text_edit(q_text_edit),
            payload(std::move(payload)), default_color(default_color),
            level_color(level_color), color_range_start(color_range_start),
    ...
    void sink_it_(const details::log_msg& msg) override
    {
        ...
        if (is_utf8_)
        {
            ...
            {
                color_range_start =
                    QString::fromUtf8(str.data(), msg.color_range_start).size();
                color_range_end =
                    QString::fromUtf8(str.data(), msg.color_range_end).size();
            }
        }
        else
        {
            payload =
                QString::fromLatin1(str.data(), static_cast<int>(str.size()));
        }
        ...
        QMetaObject::invokeMethod(
            qt_text_edit_, [params]() { invoke_method_(params); },
            Qt::AutoConnection);
    }

    void flush_() override {}

    // Add colored text to the text edit widget. This method is invoked in the
    // GUI thread. It is a static method to ensure that it is handled correctly
    // even if the sink is destroyed prematurely before it is invoked.
    ...
    static void invoke_method_(invoke_params params)
    {
        ...
        // if color range not specified or not not valid, just append the text
        // with default color
        ...
        cursor.insertText(params.payload.mid(
            params.color_range_start,
...
inline std::shared_ptr<logger> qt_color_logger_mt(
    const std::string& logger_name, QTextEdit* qt_text_edit, int max_lines,
    bool is_utf8 = false)
{
    return Factory::template create<sinks::qt_color_sink_mt>(
        logger_name, qt_text_edit, max_lines, false, is_utf8);
}
...
inline std::shared_ptr<logger> qt_color_logger_st(
    ...
    bool is_utf8 = false)
{
    return Factory::template create<sinks::qt_color_sink_st>(
        logger_name, qt_text_edit, max_lines, false, is_utf8);
}
