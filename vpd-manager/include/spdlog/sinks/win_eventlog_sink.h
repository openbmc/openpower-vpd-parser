...
{
    /** Formats an error report line: "user-message: error-code (system
     * message)" */
    static std::string format(const std::string& user_message,
    ...
    explicit win32_error(const std::string& func_name,
  ...
  public:
    ...
    static sid_t get_current_user_sid()
    {
        ...
        struct process_token_t
        {
            ...
            ::GetCurrentProcess()); // GetCurrentProcess returns pseudohandle,
                                    // no leak here!

        // Get the required size, this is expected to fail with
        // ERROR_INSUFFICIENT_BUFFER and return the token size
        DWORD tusize = 0;
        if (::GetTokenInformation(current_process_token.token_handle_,
                                  TokenUser, NULL, 0, &tusize))
        {
        ...
        if (!::GetTokenInformation(current_process_token.token_handle_,
                                   TokenUser, (LPVOID)buffer.data(), tusize,
                                   &tusize))
        {
...
class win_eventlog_sink : public base_sink<Mutex>
{
  ...
  protected:
    void sink_it_(const details::log_msg& msg) override
    {
        ...
        succeeded = static_cast<bool>(::ReportEventW(
            event_log_handle(), eventlog::get_event_type(msg),
  ...
  public:
    win_eventlog_sink(const std::string& source,
        ...
        source_(source), event_id_(event_id)
    {
        ...
        {
            // get_current_user_sid() is unlikely to fail and if it does, we can
            // still proceed without current_user_sid but in the event log the
            // record will have no user name
        }
    }
