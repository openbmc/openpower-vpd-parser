    ...
    max_files_(max_files), file_helper_{event_handlers}
{
    ...
    {
        throw_spdlog_ex(
            "rotating sink constructor: max_files arg cannot exceed MaxFiles");
    }
    ...
    const filename_t& filename, std::size_t index)
{
    ...
    return fmt_lib::format(SPDLOG_FMT_STRING(SPDLOG_FILENAME_T("{}.{}{}")),
                           basename, index, ext);
}
...
SPDLOG_INLINE void rotating_file_sink<Mutex>::set_max_size(std::size_t max_size)
{
    ...
    {
        throw_spdlog_ex(
            "rotating sink set_max_size: max_size arg cannot be zero");
    }
    max_size_ = max_size;
}
...
template <typename Mutex>
SPDLOG_INLINE void rotating_file_sink<Mutex>::set_max_files(
    std::size_t max_files)
{
    ...
    {
        throw_spdlog_ex(
            "rotating sink set_max_files: max_files arg cannot exceed 200000");
    }
    ...
    const details::log_msg& msg)
{
    ...
    // #2261). we only check the real size when new_size > max_size_ because it
