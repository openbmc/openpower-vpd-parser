...
// If building with std::format, can just use constexpr, otherwise if building
// with fmt SPDLOG_CONSTEXPR_FUNC needs to be set the same as FMT_CONSTEXPR to
// avoid situations where a constexpr function in spdlog could end up calling a
// non-constexpr function in fmt depending on the compiler If fmt determines it
// can't use constexpr, we should inline the function instead
#ifdef SPDLOG_USE_STD_FORMAT
...
SPDLOG_API const string_view_t& to_string_view(spdlog::level::level_enum l)
    SPDLOG_NOEXCEPT;
...
{
    file_event_handlers() :
        before_open(nullptr), after_open(nullptr), before_close(nullptr),
        after_close(nullptr)
    {}
