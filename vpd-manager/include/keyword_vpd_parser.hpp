...
class KeywordVpdParser : public ParserInterface
{
  public:
     ...
     * @brief Constructor
     *
     * Initializes the KeywordVpdParser with VPD data and file path.
     * Opens the VPD file for read-write access to support both parsing
     * and write operations via writeKeywordOnHardware().
     *
     * @param[in] i_kwVpdVector - VPD data to be parsed
     * @param[in] i_vpdFilePath - Path to VPD EEPROM file
     */
  ...
  private:
     ...
     * @param[in] i_keywordValue - Value to write
    ...
    /**
     * @brief API to update keyword VPD checksum
     *
     * This API is required to update the keyword VPD checksum based on the
     * current data. The checksum ensures data integrity by validating that
     * the VPD content has not been corrupted.
     *
