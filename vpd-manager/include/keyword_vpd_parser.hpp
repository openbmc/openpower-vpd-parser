...
class KeywordVpdParser : public ParserInterface
{
  public:
    ...
    KeywordVpdParser(const types::BinaryVector& i_kwVpdVector,
        ...
        m_vpdIterator(m_keywordVpdVector.begin()), m_logger(Logger::getLoggerInstance())
    {
        if (!m_vpdFilePath.empty())
        {
            try
            {
                ...
                m_vpdFileStream.open(m_vpdFilePath, std::ios::in | std::ios::out |
                                                      std::ios::binary);
            }
            catch (const std::fstream::failure& l_ex)
            {
                logging::logMessage(std::format("Failed to open VPD file for write operations: {}", l_ex.what()));
            }
        }
    }
