...
class BackupAndRestore
{
  ...
  private:
     ...
     *
     * This API gets dbus service name for given source and destination
     * service name from system config JSON.
     *
     * @param[in] i_srcInvPath - Source inventory object path.
     * @param[in] i_dstInvPath - Destination inventory object path.
    ...
    bool getSrcAndDstServiceName(
        const std::string& i_srcInvPath, const std::string& i_dstInvPath,
        uint16_t& o_errCode, std::string& o_srcServiceName,
     ...
     */
    std::tuple<std::string, std::string> getFruAndInvPaths(
     ...
     * details.
     * @param[in] i_srcVpdMap - Source IPZ VPD map.
     * @param[in] i_dstVpdMap - Destination IPZ VPD map.
    ...
    bool extractAndFindRecordInMap(
        const auto& i_aRecordKwInfo, const types::IPZVpdMap& i_srcVpdMap,
        const types::IPZVpdMap& i_dstVpdMap,
        std::tuple<std::string&, std::string&, std::string&, std::string&,
                   types::BinaryVector&>
            o_recordKeywordTuple) const noexcept;

    /**
     * @brief Retrieve the binary and string values of a keyword.
     *
     * This API returns a tuple containing the binary and string values of the
     * specified keyword. If the input VPD map is not empty, the values are
     * extracted from the map; otherwise, the keyword values are retrieved
     * through a D-Bus query.
     *
    ...
    /**
     * @brief Synchronize a keyword value to hardware.
     *
     * This API updates the specified record's keyword value on the given
     * EEPROM hardware. On success, it updates the corresponding string
     * value in the provided VPD map.
     *
