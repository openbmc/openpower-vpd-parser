...
class Manager
{
  public:
     ...
     *
     * @throw xyz.openbmc_project.Common.Error.InvalidArgument
     * @throw xyz.openbmc_project.Common.Device.Error.ReadFailure
     *
     ...
     *   the API throws an InvalidArgument error.
     * - If any VPD mismatches are found, the details are recorded in an
     *   informational PEL.
     *
     * @return true if the VPD on given EEPROM and its redundant EEPROM are the
     * same; false otherwise.
     *
     * @throws xyz.openbmc_project.Common.Error.InvalidArgument
     *         If the given EEPROM does not have a corresponding
     *         redundant EEPROM path defined in the system configuration JSON.
     */
    bool validateRedundantEeprom(const types::Path& i_fruPath) const;
  ...
  private:
     ...
     *
     * This API clubs the unexpanded location code and node number into the key
     * used to look up inventory paths in ConfigManager's map. The key is formed
