...
class ConfigManager final
{
  public:
     ...
     * The map is keyed by the node-qualified location code, formed by
     * inserting the node identifier after the 4-char prefix ("Ufcs"):
     *
     * @param[in] i_unexpandedLocationCode - Unexpanded location code
     * @param[in] i_nodeNumber - Node number
     *
     * @return Vector containing matching inventory path(s), error code
     *         otherwise.
     */
  ...
  private:
     ...
     * The node identifier is inserted after the 4-char prefix ("Ufcs")
     ...
     * @param[in] i_chassisId - Chassis ID for the FRU (e.g. "chassis0",
     *                          "chassis1").
     *
     * @return On success, returns true, otherwise sets error code
     */
    ...
    // Node-qualified unexpanded location code to inventory paths map - O(1)
    // lookup. The key is the unexpanded location code with the node identifier
    // inserted after the 4-char prefix ("Ufcs"")
    // ensures a unique key per (location-code, node) pair.
