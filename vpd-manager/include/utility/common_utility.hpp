                            ...
                            uint16_t& o_errCode) noexcept
{
    ...
    {
        std::error_code l_ec;
        if (!std::filesystem::is_directory(i_path, l_ec))
        {
            if (l_ec)
            {
        ...
        for (const auto& l_entry : l_directoryIterator)
        {
            ...
            if (skipDirectory(l_entry))
            {
                ...
                if (o_errCode == error_code::FILE_SYSTEM_ERROR)
                {
                    Logger::getLoggerInstance()->logMessage(std::format(
                        "Failed to process directory {}, error : {}.",
                        l_entry.path().string(), getErrCodeMsg(o_errCode)));
