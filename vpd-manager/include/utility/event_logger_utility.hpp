    ...
    const std::optional<std::string> i_procedure)
{
    ...
    try
    {
        ...
        const types::CalloutPriority& l_priorityEnum =
            l_invCallout.m_calloutPriority;
    ...
    const std::optional<types::UserDataEntry> i_userData2)
{
    try
    {
        ...
        const types::CalloutPriority& l_priorityEnum =
            l_i2cDevCallout.m_calloutPriority;
    ...
    [[maybe_unused]] const std::optional<std::string> i_procedure)
{
    try
    {
        ...
        {
            createSyncPel(
                i_errorType, i_severity, i_fileName, i_funcName, i_internalRc,
                i_description,
                i_userData1 ? std::optional<types::UserDataEntry>{*i_userData1}
                            : std::nullopt,
                i_userData2 ? std::optional<types::UserDataEntry>{*i_userData2}
                            : std::nullopt,
            ...
            if (std::filesystem::exists(INVENTORY_JSON_SYM_LINK, l_ec))
            {
                if (!l_ec)
                {
                    ...
                    l_calloutInvPath = jsonUtility::getInventoryObjPathFromJson(
                        l_parsedJson, (i_callouts[0].m_inventoryPath),
                        l_errCode);
                }
                else
                {
