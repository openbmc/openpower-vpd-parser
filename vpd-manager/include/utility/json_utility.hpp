    ...
    const std::string& i_baseAction, const std::string& i_flagToProcess)
{
    ...
    if (!(i_parsedConfigJson["frus"][i_vpdFilePath].at(0).contains(
            i_baseAction)))
    {
        logging::logMessage("Action [" + i_baseAction +
                            "] is not defined for file path " + i_vpdFilePath);
        return false;
    }

    if (!(i_parsedConfigJson["frus"][i_vpdFilePath]
              .at(0)[i_baseAction]
              .contains(i_flagToProcess)))
    {
    ...
    if (!(i_parsedConfigJson["frus"][i_vpdFilePath]
              .at(0)[i_baseAction][i_flagToProcess]
              .contains("gpioPresence")))
    {
        logging::logMessage(
            "gpioPresence tag is missing in config JSON for path " +
            i_vpdFilePath);
    ...
    const std::string& i_baseAction, const std::string& i_flagToProcess)
{
    ...
    {
        logging::logMessage(
            "setGpio tag is missing in config JSON for path: " + i_vpdFilePath);
