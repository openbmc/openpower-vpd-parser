...
/**
 * @brief API to get list of FRUs with "monitorPresence" = true
 *
 * @param[in] i_sysCfgJsonObj - System config JSON object.
 *
...
/**
 * @brief API which tells if the FRU's presence should be monitored
 *
 * For a given FRU, this API checks if it's presence should be monitored by
 * checking the "monitorPresence" tag.
 *
 * @param[in] i_sysCfgJsonObj - System config JSON object.
 * @param[in] i_vpdFruPath - EEPROM path.
 *
 * @return true if FRU is replaceable at standby. false otherwise.
 */
inline bool isPresenceMonitoredFru(const nlohmann::json& i_sysCfgJsonObj,
                                   const std::string& i_vpdFruPath)
{
    try
    {
    ...
    {
        logging::logMessage(
            "Failed to check if FRU's presence should be monitored, reason:" +
            std::string(l_ex.what()));
    }
