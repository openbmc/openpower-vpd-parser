 ...
 */
inline std::string getImFromPlanar() noexcept
{
    try
    {
        types::BinaryVector l_imKeyword(constants::KW_NAME_SIZE_IN_BYTES);
        ...
        l_vpdFileStream.seekg(constants::IM_KW_OFFSET, std::ios_base::beg);

        l_vpdFileStream.read(reinterpret_cast<char*>(&l_imKeyword[0]),
                             constants::KW_NAME_SIZE_IN_BYTES);
