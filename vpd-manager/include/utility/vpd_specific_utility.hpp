 ...
 * given value.
 * 
 * @param[in] i_vpdPath - Fru path (EEPROM/Dbus).
 * @param[in] i_value - State to set.
 * @param[in] i_sysCfgJsonObj - System config json object.
 * @param[out] o_errCode - To set error code in case of error.
 */
inline void setCollectionStatusProperty(const std::string& i_vpdPath, const std::string& i_value, 
    const nlohmann::json& i_sysCfgJsonObj, uint16_t& o_errCode) noexcept
{
    o_errCode = 0;
    ...
    else
    {
        ...
        {
            Logger::getLoggerInstance()->logMessage("Invalid inventory path: " + i_vpdPath);
