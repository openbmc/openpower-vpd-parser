 ...
 *
 * It extracts chassis ID from FRU inventory path and constructs appropriate
 * chassis/system inventory path based on Fcs/Mts type location code.
 *
 * @param[in] i_inventoryPath - Inventory Path.
 * @param[in] i_isFcs - Is this FCS or MTS Location code.
...
inline std::pair<std::string, std::string> getDataFromDbus(
    const std::string& i_inventoryPath, bool i_isFcs,
...
inline std::string getExpandedLocationCode(
    ...
    const std::string& i_unexpandedLocationCode, uint16_t& o_errCode)
{
    o_errCode = 0;
    ...
    try
    {
        ...
        bool l_isFcs = (l_pos != std::string::npos);
        ...
        const std::string& l_kwd1 =
            l_isFcs ? constants::kwdFC : constants::kwdTM;
        const std::string& l_kwdInterface =
            l_isFcs ? constants::vcenInf : constants::vsysInf;

        auto [l_firstKwdValue, l_secondKwdValue] =
            getDataFromDbus(i_inventoryPath, l_isFcs, l_kwdInterface, l_kwd1,
                            constants::kwdSE, o_errCode);

        if (o_errCode != 0 || l_firstKwdValue.empty() ||
            l_secondKwdValue.empty())
        {
            return i_unexpandedLocationCode;
        }

        // Validate l_firstKwdValue length before substr operation
        if (l_isFcs && l_firstKwdValue.size() < 4)
        {
        ...
        return buildExpandedLC(i_unexpandedLocationCode, l_isFcs, l_pos,
                               l_firstKwdValue, l_secondKwdValue);
    }
    catch (const std::exception& l_ex)
    {
        o_errCode = error_code::STANDARD_EXCEPTION;
