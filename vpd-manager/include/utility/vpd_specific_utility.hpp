...
inline void setCollectionStatusProperty(
    ...
    const types::VpdCollectionProgress::OperationStatus& i_value,
    const nlohmann::json& i_sysCfgJsonObj, uint16_t& o_errCode) noexcept
{
    o_errCode = 0;
    if (i_vpdPath.empty())
    ...
    if (i_value == types::VpdCollectionProgress::OperationStatus::Completed ||
        i_value == types::VpdCollectionProgress::OperationStatus::Failed)
    {
             ...
             types::VpdCollectionProgress::OperationStatus::InProgress)
    {
        l_timeStampMap.emplace(
            ...
            types::DbusVariantType{commonUtility::getCurrentTimeSinceEpoch()});
             ...
             types::VpdCollectionProgress::OperationStatus::NotStarted)
    {
    ...
    for (const auto& l_Fru : i_sysCfgJsonObj["frus"][l_eepromPath])
    {
        ...
        l_propertyValueMap.emplace(
            "Status",
            types::VpdCollectionProgress::convertOperationStatusToString(
                i_value));
        ...
        vpdSpecificUtility::insertOrMerge(
            l_interfaces, types::VpdCollectionProgress::interface,
