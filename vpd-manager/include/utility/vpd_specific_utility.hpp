    ...
    const nlohmann::json& i_sysCfgJsonObj) noexcept
{
    try
    {
        ...
        auto l_populateInterfaceMap =
            [&l_objectInterfaceMap, &l_ipzData = std::as_const(l_ipzData),
             &i_fruPath = std::as_const(i_fruPath),
             &i_sysCfgJsonObj = std::as_const(i_sysCfgJsonObj)](
                const auto& l_Fru) {
                // skip the base FRU and update inherited FRUs only
                if (l_Fru.value("inherit", true) &&
                    l_Fru.contains("inventoryPath") &&
                    i_sysCfgJsonObj["frus"][i_fruPath].at(0).contains(
                        "inventoryPath") &&
                    (l_Fru["inventoryPath"] !=
