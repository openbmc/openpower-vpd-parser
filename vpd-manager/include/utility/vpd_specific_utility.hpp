                               ...
                               size_t& vpdStartOffset, uint16_t& o_errCode)
{
    ...
    if (!vpdFileStream.is_open())
    {
        ...
        Logger::getLoggerInstance()->logMessage(std::format(
            "Failed to open EEPROM: {}, errno: {} ({})", vpdFilePath, o_errCode,
            std::generic_category().message(o_errCode)));
        return;
    }
    ...
    if (vpdFileStream.fail())
    {
        o_errCode = static_cast<uint16_t>(errno);
        Logger::getLoggerInstance()->logMessage(std::format(
            "Failed to seek EEPROM: {}, errno: {} ({})", vpdFilePath, o_errCode,
            std::generic_category().message(o_errCode)));
        return;
    }
    ...
    if (vpdFileStream.fail())
    {
        o_errCode = static_cast<uint16_t>(errno);
        Logger::getLoggerInstance()->logMessage(std::format(
            "Failed to read EEPROM: {}, errno: {} ({})", vpdFilePath, o_errCode,
            std::generic_category().message(o_errCode)));
        return;
    }
