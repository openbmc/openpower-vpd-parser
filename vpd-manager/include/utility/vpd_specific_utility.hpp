...
inline std::string getChassisId(
    const std::string& i_inventoryObjPath) noexcept
{
    try
    {
 ...
 */
/*inline std::string getExpandedLocationCode(const std::string& i_chassisObjPath,
    const std::string& i_inventoryPath, const std::string& i_unexpandedLocCode,
    uint16_t& o_errCode)
{
    o_errCode = 0;
    ...
    try
    {
        ...
        {
            return expandFcsLocationCode(i_chassisObjPath, i_inventoryPath, i_unexpandedLocCode,
                                         l_pos, o_errCode);
        }
 ...
 * @return Expanded location code string.
 */
inline std::string expandFcsLocationCode(const std::string& i_chassisObjPath, const std::string& i_inventoryPath, 
    uint16_t& o_errCode) noexcept
{
    o_errCode = 0;
    ...
    {
        /**If failed, return empty string. So the caller will not update Dbus and Dbus will continue to have what it already has.*/
        o_errCode = error_code::DBUS_FAILURE;
        return std::string{};
    }
    ...
    auto readKwdValue = [&](const std::string& kwd) -> std::string {
        auto retVal = dbusUtility::readDbusProperty(constants::pimServiceName, i_chassisObjPath,
                                                    constants::vcenInf, kwd);
        if (auto kwdVal = std::get_if<types::BinaryVector>(&retVal))
        {
    ...
    {
        Logger::getLoggerInstance()->logMessage(std::format("Failed to read kwd {} for system VPD path.", constants::kwdTM));
        return std::string{};
    }
    ...
    return l_expanded.replace(l_pos, 4 + l_suffix.length(),
...
inline std::string expandMtsLocationCode(uint16_t& o_errCode)
{
    ...
    if (o_errCode == error_code::RECEIVED_INVALID_KWD_TYPE_FROM_DBUS)
    {
        Logger::getLoggerInstance()->logMessage(std::format("Failed to read kwd {} for system VPD path.", constants::kwdSE));
        return std::string{};
    }
