 ...
 *
 * Checks if VCEN record is present in the parsed VPD map:
 * - If VCEN is found, FC and SE keywords are read from the parsed VPD map.
 *   No D-Bus call is made. If keyword read fails or values are empty, error
 *   is returned immediately.
 * - If VCEN is not found in the parsed VPD map, FC and SE keywords are read
 *   from D-Bus (vcenInf).
 *
    ...
    const types::VPDMapVariant& i_parsedVpdMap, size_t i_pos)
{
    ...
    const auto l_vcenItr = l_ipzMap ? l_ipzMap->find(constants::recVCEN)
                                    : types::IPZVpdMap::const_iterator{};
    const bool l_vcenInMap = l_ipzMap && (l_vcenItr != l_ipzMap->end());

    if (l_vcenInMap)
    {
        // VCEN found in parsed VPD map — read FC and SE from it. No D-Bus.
    ...
    {
        // VCEN not in parsed VPD map — read FC and SE from D-Bus.
        const auto l_chassisId = getChassisId(i_inventoryPath, l_errCode);
        if (l_chassisId.empty() && l_errCode)
        {
            return std::unexpected(l_errCode);
        }
