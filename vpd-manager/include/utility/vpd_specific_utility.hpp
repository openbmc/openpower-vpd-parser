 ...
 * @param[in] i_commonInterfaceJson - Interface JSON object.
 * @param[out] o_errCode - To set error code in case of error.
 *
    ...
    try
    {
        ...
        auto l_populateObjectInterfaceMap =
            [&l_objectInterfaceMap, &l_interfaceMap = std::as_const(
                ...
                if ((l_Fru.value("inherit", true) || l_Fru.value("inheritCI", false)) &&
 ...
 * @param[out] o_errCode - To set error code in case of error.
 */
inline void updateExtraInterafaceProperties(
    const std::string& i_fruPath,
    const types::WriteVpdParams& i_paramsToWriteData,
    const nlohmann::json& i_sysCfgJsonObj, uint16_t& o_errCode) noexcept
{
    const std::vector<std::string> l_listofInterfacesToUpdate{constants::assetInf};
    o_errCode = 0;
    try
    {
        if (i_fruPath.empty() || i_sysCfgJsonObj.empty())
        ...
        auto l_populateObjectInterfaceMap = [&l_listofInterfacesToUpdate,
                                             ...
                                             &i_paramsToWriteData,
                                             &o_errCode](const auto& l_inventoryItem) {
            if (l_inventoryItem.contains("extraInterfaces"))
            {
                ...
                {
                    l_objectInterfaceMap.emplace(
                        sdbusplus::message::object_path{l_inventoryItem["inventoryPath"]},
                        l_interfaceMap);
                }
            }
        };
        ...
        //  iterate through all inventory paths for given EEPROM path,
        //  if for an inventory path, "inherit" or "inheritCI" tag is true
        //  update the inventory path's corresponding extra interface(s) property
        std::for_each(i_sysCfgJsonObj["frus"][i_fruPath].begin(),
                      i_sysCfgJsonObj["frus"][i_fruPath].end(),
                      l_populateObjectInterfaceMap);
