 ...
 *
 * Supports: Ufcs-ND0-SCx, Ufcs-ND0-SCxx, Ufcs-ND0-NDx, Ufcs-ND0-NDxx (with optional suffixes)
 *
    ...
    const types::VPDMapVariant& parsedVpdMap, uint16_t& o_errCode)
{
    ...
    try
    {
        ...
        const std::string& recordName = isFcs ? constants::recVCEN : constants::recVSYS;
        const std::string& kwd1 = isFcs ? constants::kwdFC : constants::kwdTM;
        const std::string& kwdInterface = isFcs ? constants::vcenInf : constants::vsysInf;
        ...
        if (auto ipzVpdMap = std::get_if<types::IPZVpdMap>(&parsedVpdMap))
        {
            ...
            if (!ipzVpdMap->empty())
            {
                ...
                if (recordItr != ipzVpdMap->end())
                {
                    ...
                    firstKwdValue = getKwVal(recordItr->second, kwd1, o_errCode);
                    if (o_errCode != 0) return unexpandedLocationCode;

                    secondKwdValue = getKwVal(recordItr->second, kwd2, o_errCode);
                    if (o_errCode != 0) return unexpandedLocationCode;
