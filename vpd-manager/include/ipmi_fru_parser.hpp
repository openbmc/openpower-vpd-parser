...
class IpmiFruParser : public ParserInterface
{
  public:
    ...
    explicit IpmiFruParser(types::BinaryVector i_vpdVector,
        ...
        m_vpdFilePath(i_vpdFilePath), m_logger(Logger::getLoggerInstance())
    {
    ...
    int writeKeywordOnHardware(
        const types::WriteVpdParams i_params) override;
  ...
  private:
     ...
     * @param[in] i_langCode - Language code of the enclosing area (0 = English).
     *
     ...
     * @param[in] i_areaLength - Area length in 8-byte units (from header byte 1).
     *
