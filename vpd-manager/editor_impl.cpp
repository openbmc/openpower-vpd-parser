...
{
    std::filesystem::path path{"/tmp/vpdBackup"};
    ...
    if (std::filesystem::exists(path))
    {
        ...
        {
            throw runtime_error(
                "Failed to open vpd file path in /tmp/vpdBackup. "
...
void EditorImpl::updateData(const Binary& kwdData)
{
    ...
    if (std::filesystem::exists("/tmp/vpdBackup/updateVpdDataBackup.txt"))
    {
        if (std::filesystem::remove("/tmp/vpdBackup/updateVpdDataBackup.txt"))
        {
