...
Worker::Worker(std::string pathToConfigJson, uint8_t i_maxThreadCount,
    ...
    m_vpdCollectionMode(i_vpdCollectionMode), m_logger(Logger::getLoggerInstance())
{
    ...
    const std::string& i_vpdFilePath)
{
    ...
    try
    {
        ...
        vpdSpecificUtility::setCollectionStatusProperty(i_vpdFilePath,
                                    constants::vpdCollectionInProgress, m_parsedJson, l_errCode);
        if(l_errCode)
        {
            m_logger->logMessage("Failed to set collection status for path " + i_vpdFilePath + "Reason: " 
                + commonUtility::getErrCodeMsg(l_errCode));
        }
    ...
    {
        vpdSpecificUtility::setCollectionStatusProperty(i_vpdFilePath,
                                    constants::vpdCollectionFailed, m_parsedJson, l_errCode);
        if(l_errCode)
        {
            m_logger->logMessage("Failed to set collection status for path " + i_vpdFilePath + "Reason: " 
                + commonUtility::getErrCodeMsg(l_errCode));
    ...
    vpdSpecificUtility::setCollectionStatusProperty(i_vpdFilePath,
                                constants::vpdCollectionCompleted, m_parsedJson, l_errCode);
    if(l_errCode)
    {
        m_logger->logMessage("Failed to set collection status for path " + i_vpdFilePath + "Reason: " 
            + commonUtility::getErrCodeMsg(l_errCode));
    ...
    const sdbusplus::message::object_path& i_dbusObjPath)
{
    ...
    try
    {
        ...
        vpdSpecificUtility::setCollectionStatusProperty(l_fruPath,
                                constants::vpdCollectionInProgress, m_parsedJson, l_errCode);
        if(l_errCode)
        {
            m_logger->logMessage("Failed to set collection status for path " + l_fruPath + "Reason: " 
                + commonUtility::getErrCodeMsg(l_errCode));
        }
    ...
    {
        vpdSpecificUtility::setCollectionStatusProperty(l_fruPath, constants::vpdCollectionCompleted, m_parsedJson, l_errCode);
        if(l_errCode)
        {
            m_logger->logMessage("Failed to set collection status for path " + l_fruPath + "Reason: " 
                + commonUtility::getErrCodeMsg(l_errCode));
