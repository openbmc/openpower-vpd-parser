    ...
    for (const auto& interfacesPropPair : interfaceJson.items())
    {
        ...
        for (const auto& propValuePair : interfacesPropPair.value().items())
        {
            ...
            else if (propValuePair.value().is_array())
            {
                ...
                {
                    m_logger->logMessage("Type exception: " + e.what(),placeHolder::COLLECTION);
                }
            }
            ...
            else if (propValuePair.value().is_object())
            {
                             ...
                             std::get_if<types::KeywordVpdMap>(&parsedVpdMap))
                {
                    if (!keyword.empty() && (*kwdVpdMap).count(keyword))
                    {
                        ...
                        {
                            m_logger->logMessage(
                                "Unknown keyword found, Keywrod = " + keyword, placeHolder::COLLECTION);
                        }
                    }
                }
            }
                              ...
                              uint16_t& i_errCode)
{
        ...
        (i_flagToProcess.compare("collection") == constants::STR_CMP_SUCCESS))
    {
        ...
        if (!inventoryPath.empty())
        {
            ...
            {
                m_logger->logMessage(
                    "Call to PIM failed for file " + i_vpdFilePath, placeholder::COLLECTION);
            }
        }
        else
        {
            m_logger->logMessage(
                "Inventory path is empty in Json for file " + i_vpdFilePath, placeholder::COLLECTION);
        }
    ...
    const std::optional<types::VPDMapVariant> i_parsedVpd)
{
    ...
    {
        m_logger->logMessage(
            "Invalid input parameter. Abort processing post action", placeHolder::COLLECTION);
        return false;
    }
            ...
            .contains("ccin"))
    {
        ...
        {
            m_logger->logMessage("Empty VPD Map",placeHolder::COLLECTION);
            return false;
        }
                                        ...
                                        l_errCode))
    {
        m_logger->logMessage(
            ...
            " . Reason: " + commonUtility::getErrCodeMsg(l_errCode),placeHolder::COLLECTION);
                                          ...
                                          "preAction", "collection", l_errCode))
        {
            ...
            if (!processPreAction(i_vpdFilePath, "collection", l_errCode))
            {
                if (l_errCode == error_code::DEVICE_NOT_PRESENT)
                {
                    m_logger->logMessage(
                        commonUtility::getErrCodeMsg(l_errCode) +
                        i_vpdFilePath,placeHolder::COLLECTION);
    ...
    try
    {
        ...
        else
        {
            m_logger->logMessage("Empty parsedVpdMap recieved for path [" +
                                i_vpdFilePath + "]. Check PEL for reason.", placeHolder::COLLECTION);
...
void Worker::performVpdRecollection()
{
    ...
    catch (const std::exception& l_ex)
    {
        // TODO Log PEL
        m_logger->logMessage(
            "VPD recollection failed with error: " + std::string(l_ex.what()),placeHolder::COLLECTION);
    }
}
    ...
    const sdbusplus::message::object_path& i_dbusObjPath)
{
    ...
    try
    {
        ...
        if (m_parsedJson.empty())
        {
            m_logger->logMessage(
                ...
                std::string(i_dbusObjPath), placeHolder::COLLECTION);
            return;
        }
        ...
        if (l_fruPath.empty())
        {
            if (l_errCode)
            {
                m_logger->logMessage(
                    ...
                    "], error : " + commonUtility::getErrCodeMsg(l_errCode) +
                    " Aborting single FRU VPD collection.", placeHolder::COLLECTION);
                return;
            }
    ...
    catch (const std::exception& l_error)
    {
        ...
        // TODO: Log PEL
        m_logger->logMessage(std::string(l_error.what()), placeHolder::COLLECTION);
    }
}
