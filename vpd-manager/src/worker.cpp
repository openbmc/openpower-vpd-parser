...
void Worker::setDeviceTreeAndJson()
{
    ...
    { // Skipping setting device tree as either devtree info is missing from
      // Json or it is rightly set.
        ...
        {
            if (jsonUtility::isBackupAndRestoreRequired(m_parsedJson, l_errCode))
            {
                performBackupAndRestore(parsedVpdMap);
            }
            else if(l_errCode)
            {
                logging::logMessage("Cannot perform backup and restore. Reason : " + vpdSpecificUtility::getErrCodeMsg(l_errCode));
            }
        }
