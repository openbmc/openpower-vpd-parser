                              ...
                              uint16_t& i_errCode)
{
        ...
        (i_flagToProcess.compare("collection") == constants::STR_CMP_SUCCESS))
    {
	    ...
	    logger::logMessage("DBG: processPreAction via callDbusMethod");
    ...
    const std::string& i_vpdFilePath)
{
    ...
    try
    {
        ...
        if (!std::holds_alternative<std::monostate>(parsedVpdMap))
        {
            ...
            // Call dbus method to update on dbus
	    logger::logMessage("DBG: parseAndPublishVPD via callDbusMethod");
    ...
    try
    {
        ...
        if (auto l_value = std::get_if<bool>(&l_presentPropValue))
        {
            ...
            else
            {
                ...
                // Call dbus method to update on dbus
	        logger::logMessage("DBG: deleteFruVpd via callDbusMethod");
