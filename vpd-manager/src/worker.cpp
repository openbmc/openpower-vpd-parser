...
void Worker::deleteFruVpd(const std::string& i_dbusObjPath)
{
    ...
    const std::string& l_fruPath =
        jsonUtility::getFruPathFromJson(l_chassisBasedJson, i_dbusObjPath, l_errCode);
    ...
    try
    {
        if (jsonUtility::isActionRequired(l_chassisBasedJson, l_fruPath, "preAction",
                                          "deletion", l_errCode))
        {
        ...
        vpdSpecificUtility::resetObjTreeVpd(l_fruPath, l_chassisBasedJson, l_errCode);
        ...
        if (jsonUtility::isActionRequired(l_chassisBasedJson, l_fruPath, "postAction",
                                          "deletion", l_errCode))
        {
    ...
    catch (const std::exception& l_ex)
    {
        ...
        if (jsonUtility::isActionRequired(l_chassisBasedJson, l_fruPath,
                                          "postFailAction", "deletion",
                                          l_errCode))
        {
            if (!jsonUtility::executePostFailAction(l_chassisBasedJson, l_fruPath,
                                                    "deletion", l_errCode))
            {
    ...
    const sdbusplus::message::object_path& i_dbusObjPath)
{
    ...
    try
    {
        ...
        l_fruPath = jsonUtility::getFruPathFromJson(l_chassisBasedJson, i_dbusObjPath,
                                                    l_errCode);
        ...
        if (dbusUtility::isHostRunning())
        {
            ...
            bool isFruReplaceableAtRuntime =
                jsonUtility::isFruReplaceableAtRuntime(l_chassisBasedJson, l_fruPath,
                                                       l_errCode);
        ...
        else if (dbusUtility::isBMCReady())
        {
            ...
            bool isFruReplaceableAtStandby =
                jsonUtility::isFruReplaceableAtStandby(l_chassisBasedJson, l_fruPath,
                                                       l_errCode);
            ...
            if (!isFruReplaceableAtStandby && (!isFruReplaceableAtRuntime))
            {
                logging::logMessage(
                    ...
                    std::string(i_dbusObjPath));
		logging::logMessage("Enabled for testing");
                //return;
            }
        }

        vpdSpecificUtility::setCollectionStatusProperty(
            l_fruPath, types::VpdCollectionStatus::InProgress, l_chassisBasedJson,
            l_errCode);
        if (l_errCode)
        {
            m_logger->logMessage(
        ...
        vpdSpecificUtility::setCollectionStatusProperty(
            l_fruPath, types::VpdCollectionStatus::Completed, l_chassisBasedJson,
            l_errCode);
        if (l_errCode)
        {
            m_logger->logMessage(
    ...
    catch (const std::exception& l_error)
    {
        ...
        vpdSpecificUtility::setCollectionStatusProperty(
            l_fruPath, types::VpdCollectionStatus::Failed, l_chassisBasedJson,
            l_errCode);
        if (l_errCode)
        {
