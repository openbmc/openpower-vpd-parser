    ...
    try
    {
        ...
        if (auto l_value = std::get_if<bool>(&l_presentPropValue))
        {
            ...
            else
            {
                ...
                if (l_errCode == error_code::FRU_PRESENCE_NOT_HANDLED)
                {
                    throw std::runtime_error(
                        "Given FRU is present but its presence is not handled by vpd-manager");
                }
                else if (l_errCode)
                {
                    throw std::runtime_error(
                        "Failed to clear data under PIM for FRU, error : " +
                        commonUtility::getErrCodeMsg(l_errCode));
                }
    ...
    const sdbusplus::message::object_path& i_dbusObjPath)
{
    ...
    catch (const std::exception& l_error)
    {
        ...
        if (l_errCode)
        {
            l_errMsg += "Failed to set collection status for path " +
                        l_fruPath +
                        "Reason: " + commonUtility::getErrCodeMsg(l_errCode);
        }
        // TODO: Log PEL
