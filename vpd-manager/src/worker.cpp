...
std::tuple<bool, std::string> Worker::processFruCollection(
    const std::string& i_fruPath, const nlohmann::json& i_cfgJsonObj, uint16_t& o_errCode)
{
    ...
    try
    {
        ...
        l_presence = dbusUtility::isInventoryPresent(i_cfgJsonObj["frus"][i_fruPath].at(0)["inventoryPath"]);

        if (std::get<0>(l_parseresult))
        {
            return std::make_tuple(l_presence, constants::vpdCollectionCompleted);
        }
        else
        {
    ...
    catch(const std::exception& l_ex)
    {
        ...
        m_logger->logMessage(std::format("Failed to process FRU VPD collection, error : {}.", l_ex.what()));
    }
