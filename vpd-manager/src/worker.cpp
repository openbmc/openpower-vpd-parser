                              ...
                              uint16_t& o_errCode)
{
    ...
    try
    {
        types::PreActionResult l_fruPreActionResult;
        ...
        if (!l_actionResult.m_success)
        {
            ...
            l_fruPreActionResult.m_errorCode =
                l_actionResult.m_failedTagErrorCode;
        }

        const auto l_invObjPath = jsonUtility::getInventoryObjPathFromJson(
            m_parsedJson, i_vpdFruPath, o_errCode);

        if (o_errCode != 0 || l_invObjPath.empty())
        {
            m_logger->logMessage(
                "Failed to get inventory object path from JSON for FRU [" +
                    i_vpdFruPath + "], OR it is empty. error: " +
        ...
        switch (l_actionResult.m_presenceStatus)
        {
            ...
            {
                setPresentProperty(l_invObjPath, true);
                break;
            }
            ...
            default:
            {
                // Implies not supported presence state
                // Still its VPD can be collected, log a message and continue.
                m_logger->logMessage(std::format(
                    "Presence status received for FRU: {} is not supported, Presence Status: [{}]",
                    i_vpdFruPath,
                    static_cast<int>(l_actionResult.m_presenceStatus)));
                break;
            }
        }
