...
void Worker::performVpdRecollection(const nlohmann::json i_chassisBasedJson)
{
    try
    {
        ...
        const auto& l_frusReplaceableAtStandby =
            jsonUtility::getListOfFrusReplaceableAtStandby(i_chassisBasedJson,
                                                           l_errCode);
        ...
        for (const auto& l_fruInventoryPath : l_frusReplaceableAtStandby)
        {
            ...
            collectSingleFruVpd(i_chassisBasedJson, l_fruInventoryPath);
        }
        return;
    }
                                 ...
                                 const sdbusplus::object_path& i_dbusObjPath)
{
    ...
    try
    {
        ...
        l_errCode = 0;
        auto [l_present, l_collectionStatus] =
            collectFruVpd(l_fruPath, i_chassisBasedJson, l_errCode);

        if (l_errCode)
        {
            m_logger->logMessage(std::format(
                "VPD collection for EEPROM [{}] failed. "
                "Present: {}, Status: {}, ErrorCode: {}",
                l_fruPath, l_present, l_collectionStatus, l_errCode));
        }
