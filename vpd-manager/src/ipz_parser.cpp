    ...
    const types::WriteVpdParams i_paramsToWriteData)
{
    ...
    try
    {
        ...
        // Restrict updating of IM value if it is not in supported list of IM values.
        if (l_keywordName == constants::kwdIM)
        {
            ...
            if (config::systemType.find(l_imData.str()) == config::systemType.end())
            {
                throw DataException( __FUNCTION__ + std::string(": Given IM keyword value does not map to any system type."));
            }
        }
