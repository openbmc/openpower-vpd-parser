...
std::vector<std::string> IpzVpdParser::getMissingDataFromMap(
    ...
    [[maybe_unused]] const types::IpzVpdMapVariant& i_mapToCompare)
{
    ...
    const std::shared_ptr<vpd::ParserInterface>& i_redundantParser) noexcept
{
    ...
    try
    {
        ...
        std::vector<std::string> l_missingRecordsInPrimary{getMissingDataFromMap(l_redundantIpzVpdMap, l_primaryIpzVpdMap)};
        ...
        if (!l_areEqual)
        {
            ...
            if (!l_missingRecordsInPrimary.empty())
            {
                const auto l_joined = l_missingRecordsInPrimary |
            ...
            if (!l_missingKeywordsInPrimary.empty())
            {
                l_error += " Missing Keyword(s) on primary VPD: " +
                           vpdSpecificUtility::getInStringFormat(
                               l_missingKeywordsInPrimary);
            }

            if (!l_missingRecordsInRedundant.empty())
            {
                const auto l_joined = l_missingRecordsInRedundant |
                                      std::views::join_with(std::string(", "));
            ...
            if (!l_mismatchedVpd.empty())
            {
                l_error +=
                    ...
                    vpdSpecificUtility::getInStringFormat(l_mismatchedVpd);
            }
