                        ...
                        const std::source_location& i_location) noexcept
{
    ...
    try
    {
		...
		std::vector<types::InventoryCalloutData> l_invCallout;

		if (l_calloutOpt &&
                    std::holds_alternative<types::InventoryCalloutData>(*l_calloutOpt))
                {
                    l_invCallout.push_back(std::get<types::InventoryCalloutData>(*l_calloutOpt));
		}

                EventLogger::createAsyncPelWithInventoryCallout(
                    m_connection, std::get<0>(*i_pelTuple), l_severity,
                    l_invCallout, i_location.file_name(),
                    i_location.function_name(), std::get<2>(*i_pelTuple),
                    std::string(i_message), std::get<3>(*i_pelTuple),
                    std::get<4>(*i_pelTuple), std::get<5>(*i_pelTuple),
                    std::get<6>(*i_pelTuple));
        ...
        else if (i_placeHolder == PlaceHolder::ASYNC_PEL_WITH_I2C_DEV_CALLOUT)
        {
            if (i_pelTuple.has_value())
            {
                // By default set severity to informational
                ...
                auto& l_calloutOpt = std::get<7>(*i_pelTuple);
                std::vector<types::DeviceCalloutData> l_i2cDevCallout;

                if (l_calloutOpt &&
                    std::holds_alternative<types::DeviceCalloutData>(*l_calloutOpt))
                {
                    l_i2cDevCallout.push_back(
                        std::get<types::DeviceCalloutData>(*l_calloutOpt));
                }

                EventLogger::createAsyncPelWithI2cDeviceCallout(
                    m_connection, std::get<0>(*i_pelTuple), l_severity,
                    l_i2cDevCallout, i_location.file_name(),
                    i_location.function_name(), std::get<2>(*i_pelTuple),
                    std::string(i_message), std::get<3>(*i_pelTuple),
            ...
            std::println(
                "Pel info tuple required to log async PEL with I2C device callouts for message {}",
                l_log.str());
        }
        else if (i_placeHolder == PlaceHolder::SYNC_PEL_WITH_I2C_DEV_CALLOUT)
        {
            if (i_pelTuple.has_value())
            {
		//TODO: Refactor in another commit- move all PEL's severity out of if-else.
                // By default set severity to informational
                const types::SeverityType l_severity =
                    std::get<1>(*i_pelTuple).has_value()
                        ? std::get<1>(*i_pelTuple).value()
		...
		if (l_calloutOpt &&
                    std::holds_alternative<types::DeviceCalloutData>(*l_calloutOpt))
                {
                    l_i2cDevCallout.push_back(std::get<types::DeviceCalloutData>(*l_calloutOpt));
		}


                EventLogger::createSyncPelWithI2cDeviceCallout(
                    std::get<0>(*i_pelTuple), l_severity,
                    l_i2cDevCallout, i_location.file_name(),
                    i_location.function_name(), std::get<2>(*i_pelTuple),
                    std::string(i_message), std::get<3>(*i_pelTuple),
            ...
            std::println(
                "Pel info tuple required to log sync PEL with I2C device callouts for message {}",
                l_log.str());
        }
        else
        {
