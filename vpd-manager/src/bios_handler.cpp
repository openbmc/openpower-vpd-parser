...
IbmBiosHandler::IbmBiosHandler(const std::shared_ptr<Manager>& i_manager) :
    m_manager(i_manager), m_logger(Logger::getLoggerInstance()), m_isValid(false)
{
    ...
    if (!l_workerObj)
    {
        m_logger->logMessage(
                std::string(
                    "Error Worker object is null in IbmBiosHandler."),
    ...
    if (l_sysCfgJsonObj.empty())
    {
        m_logger->logMessage(
                std::string(
                    "System Configuration JSON is empty."),
                PlaceHolder::PEL, &l_pel);
        return;
    }
    ...
    if (l_biosHandlerJsonCfgFilePath.empty())
    {
        m_logger->logMessage(
                std::string(
                    "BiosHandlerJsonPath key is missing in config file."),
                PlaceHolder::PEL, &l_pel);
        return;
    }
...
void IbmBiosHandler::backUpOrRestoreBiosAttributes()
{
    ...
    for (const auto& entry : m_biosConfigJson["biosRecordKwMap"])
    {
        ...
        if (it != handlers.end())
        {
            std::cout << "Got the attribute name " << attrName << " " << std::endl;
...
void IbmBiosHandler::processFieldCoreOverride()
{
    ...
    std::cout<<"inside processFieldCoreOverride"<<std::endl;
    if (!m_biosConfigJson.contains("biosRecordKwMap"))
    {
        m_logger->logMessage(
...
void IbmBiosHandler::saveFcoToVpd(int64_t i_fcoInBios)
{
    ...
    std::cout<<"inside saveFcoToVpd"<<std::endl;
    std::string l_keyWordName;
    std::string l_RecordName;
...
void IbmBiosHandler::saveAmmToVpd(const std::string& i_memoryMirrorMode)
{
    ...
    std::cout<<"inside saveAmmToVpd"<<std::endl;
    std::string l_keyWordName;
    std::string l_RecordName;
...
void IbmBiosHandler::processActiveMemoryMirror()
{
    ...
    std::cout<<"inside processActiveMemoryMirror"<<std::endl;
    for (const auto& entry : m_biosConfigJson["biosRecordKwMap"])
    {
        // We look for the attribute name this function is "assigned" to
        if (entry.value("biosAttributeName", "") == "hb_memory_mirror_mode")
    ...
    const std::string& i_createDefaultLparVal)
{
    ...
    std::cout<<"inside saveCreateDefaultLparToVpd"<<std::endl;
    std::string l_keyWordName;
    std::string l_RecordName;
...
void IbmBiosHandler::processCreateDefaultLpar()
{
    ...
    std::cout<<"inside processCreateDefaultLpar"<<std::endl;
    for (const auto& entry : m_biosConfigJson["biosRecordKwMap"])
    {
        // We look for the attribute name this function is "assigned" to
        if (entry.value("biosAttributeName", "") == "pvm_create_default_lpar")
...
void IbmBiosHandler::saveClearNvramToVpd(const std::string& i_clearNvramVal)
{
    ...
    std::string l_keyWordName;
    std::string l_RecordName;
     std::cout<<"inside saveClearNvramToVpd"<<std::endl;
    if (!m_isValid)
    {
        m_logger->logMessage(
            "Bios config JSON is empty or missing necessary tag(s), return");
...
void IbmBiosHandler::processClearNvram()
{
    std::string l_keyWordName;
    std::string l_RecordName;
    std::cout<<"inside processClearNvram"<<std::endl;
    if (!m_isValid)
    {
        m_logger->logMessage(
            "Bios config JSON is empty or missing necessary tag(s), return");
...
void IbmBiosHandler::saveKeepAndClearToVpd(const std::string& i_KeepAndClearVal)
{
    ...
    std::cout<<"inside saveKeepAndClearToVpd"<<std::endl;
    std::string l_keyWordName;
    std::string l_RecordName;
...
void IbmBiosHandler::processKeepAndClear()
{
    std::string l_keyWordName;
    std::string l_RecordName;
    std::cout<<"inside processKeepAndClear"<<std::endl;
    if(!m_isValid)
    {
        m_logger->logMessage(
            "Bios config JSON is empty or missing necessary tag(s), return");
