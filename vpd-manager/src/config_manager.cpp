    ...
    std::optional<nlohmann::json> i_commonJsonObj) noexcept
{
    try
    {
        ...
        // assume system inventory path is always under chassis0
        // @todo: handle cable inventory paths once they are modelled in system
        // config JSON
        const auto& l_chassisId = l_baseInvObjPath == constants::systemInvPath
                                      ? "chassis0"
                                      : getChassisId(l_baseInvObjPath);
