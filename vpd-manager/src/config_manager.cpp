        ...
        const std::optional<std::string>& i_vpdPath) const noexcept
{
    ...
    if ((*i_vpdPath).starts_with(std::string_view(constants::pimPath)))
    {
        ...
        if (l_chassisId.empty())
        {
            ...
            if (isObjectPathInJson(*i_vpdPath))
            {
                return std::cref(m_systemConfigJson);
            }
        }
    ...
    const std::optional<nlohmann::json>& i_commonJsonObj) noexcept
{
    try
    {
        ...
        {
            // see if the given object path is in the system configuration JSON
            // if yes, no need to add this path to maps. getJsonObj() will
            // return full system configuration JSON for this path.
            if (isObjectPathInJson(l_baseInvObjPath))
            {
                return true;
            }
...
bool ConfigManager::isObjectPathInJson(
    const std::string& i_dbusObjPath) const noexcept
{
    ...
    if (i_dbusObjPath.empty() || !i_dbusObjPath.starts_with(constants::pimPath))
    {
        return false;
    }
    ...
    try
    {
        for (const auto& l_fruEntry : m_systemConfigJson["frus"].items())
        {
            ...
            return (l_subFruJsonArray.end() !=
                    std::ranges::find_if(
                        l_subFruJsonArray,
                        [&i_dbusObjPath](const nlohmann::json& i_subFruJson) {
                            return i_subFruJson.value("inventoryPath", "") ==
                                   i_dbusObjPath;
                        }));
        }
    }
