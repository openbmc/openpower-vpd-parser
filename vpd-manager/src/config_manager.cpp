    ...
    const std::optional<nlohmann::json>& i_commonJsonObj) noexcept
{
    try
    {
        ...
        return buildLocCodeToInvPathMapForFru(l_subFruJsonArray);
    }
    catch (const std::exception& l_ex)
    {
        m_logger->logMessage(
...
std::expected<bool, error_code> ConfigManager::buildLocCodeToInvPathMapForFru(
    ...
    try
    {
        for (const auto& l_subFruJson : i_subFruJsonArray)
        {
            ...
            if (l_subFruJson.contains("inventoryPath"))
            {
                ...
                {
                    m_unexpandedLocCodeToInvPathMap[l_locationCodeRes.value()]
