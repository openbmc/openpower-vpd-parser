...
std::string
    IpmiFruParser::decodeField(uint8_t i_typelen,
                               types::BinaryVector::const_iterator i_data,
                               uint8_t i_langCode) const
{
    const uint8_t l_typeCode =
        static_cast<uint8_t>((i_typelen & TYPELEN_TYPE_MASK) >> TYPELEN_TYPE_SHIFT);
    ...
    size_t& o_offset, size_t& o_rawLen) const
{
    ...
    const size_t l_len =
        static_cast<size_t>(l_typelen & TYPELEN_LEN_MASK);

    if (io_it + static_cast<ptrdiff_t>(l_len) > i_areaEnd)
    {
        throw DataException(
            std::format("IpmiFruParser: field length {} extends past area end",
                        l_len));
    }

    o_offset = static_cast<size_t>(
        std::distance(m_vpdVector.cbegin(), io_it));
    o_rawLen = l_len;
...
void IpmiFruParser::parseChassisInfoArea(size_t i_offset)
{
    ...
    {
        throw DataException("IpmiFruParser: Chassis Info Area extends past buffer");
    }

    validateChecksum(m_vpdVector.cbegin() + static_cast<ptrdiff_t>(i_offset),
                     m_vpdVector.cbegin() +
                         static_cast<ptrdiff_t>(i_offset + l_areaLen - 1),
    ...
    m_fruMap["CHASSIS_Type"] =
        types::BinaryVector{l_chassisType};

    auto l_it =
        m_vpdVector.cbegin() + static_cast<ptrdiff_t>(i_offset + 3);
    ...
    for (const auto& l_fname : l_predefined)
    {
        ...
        m_fruMap[l_fname] =
            types::BinaryVector(l_val->begin(), l_val->end());
        m_fieldOffsets[l_fname] = {l_off, l_rawLen};
        m_fieldAreaInfo[l_fname] = {i_offset,
...
void IpmiFruParser::parseBoardInfoArea(size_t i_offset)
{
    ...
    if (i_offset + l_areaLen > m_vpdVector.size())
    {
        throw DataException("IpmiFruParser: Board Info Area extends past buffer");
    }
    ...
    m_fruMap["BOARD_MfgDateTime"] =
        types::BinaryVector(m_vpdVector.cbegin() + static_cast<ptrdiff_t>(l_mfgDateOffset),
    ...
    auto l_it =
        m_vpdVector.cbegin() + static_cast<ptrdiff_t>(i_offset + 6);
    const auto l_areaEnd =
        m_vpdVector.cbegin() +
        static_cast<ptrdiff_t>(i_offset + l_areaLen - 1);
...
void IpmiFruParser::parseProductInfoArea(size_t i_offset)
{
    ...
    if (i_offset + l_areaLen > m_vpdVector.size())
    {
        throw DataException("IpmiFruParser: Product Info Area extends past buffer");
    }
...
void IpmiFruParser::parseMultiRecordArea(size_t i_offset)
{
    ...
    while (l_pos + MULTIRECORD_HDR_SIZE <= m_vpdVector.size())
    {
        ...
        validateChecksum(m_vpdVector.cbegin() + static_cast<ptrdiff_t>(l_pos),
                         m_vpdVector.cbegin() +
                             static_cast<ptrdiff_t>(l_pos + MULTIRECORD_HDR_SIZE - 1),
                         l_hdrChecksum,
                         std::format("MultiRecord header {}", l_recIdx));
        ...
        if (l_recLen > 0)
        {
            validateChecksum(
                ...
                l_recChecksum,
                std::format("MultiRecord {} data", l_recIdx));
        }

        const std::string l_keyData =
            "MULTIRECORD_" + std::to_string(l_recIdx);
        const std::string l_keyType = l_keyData + "_TypeId";

        m_fruMap[l_keyType] = types::BinaryVector{l_typeId};
        m_fruMap[l_keyData] =
            types::BinaryVector(m_vpdVector.cbegin() + static_cast<ptrdiff_t>(l_dataStart),
                                m_vpdVector.cbegin() +
...
void IpmiFruParser::rewriteAreaChecksum(size_t i_areaStart,
                                        size_t i_areaLength)
{
    if (i_areaLength == 0)
    {
    ...
    if (m_vpdFileStream.is_open())
    {
        ...
        m_vpdFileStream.write(
            reinterpret_cast<const char*>(&l_newChecksum), 1);
...
types::VPDMapVariant IpmiFruParser::parse()
{
    ...
    if (l_internalOffset != 0)
    {
        try
        {
            ...
            for (size_t l_candidate :
                 {l_chassisOffset, l_boardOffset, l_productOffset,
                  l_multirecOffset})
            {
...
int IpmiFruParser::writeKeywordOnHardware(const types::WriteVpdParams i_params)
{
    ...
    const size_t l_toWrite =
        std::min(l_value.size(), l_capacity);

    // 1. Update the in-memory vector first so checksum computation uses the
    //    new data (mirrors the pattern in KeywordVpdParser::writeKeywordOnHardware).
    std::copy_n(l_value.begin(), l_toWrite,
                m_vpdVector.begin() + static_cast<ptrdiff_t>(l_byteOffset));

    // 2. Recompute and apply the area checksum to both in-memory vector and file.
    ...
    //    write just the field bytes (checksum already written by rewriteAreaChecksum).
