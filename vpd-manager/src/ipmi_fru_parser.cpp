...
uint8_t IpmiFruParser::computeChecksum(
    ...
    IpmiFruParser::processCommonHeader() noexcept
{
    ...
    if (computeChecksum(m_vpdVector.cbegin(),
                        m_vpdVector.cbegin() +
                            static_cast<ptrdiff_t>(COMMON_HEADER_SIZE)) !=
        constants::VALUE_0)
    {
...
types::VPDMapVariant IpmiFruParser::parse()
{
    ...
    if (!l_headerStatus)
    {
        throw DataException("IPMI FRU: Common Header checksum mismatch");
    }
    ...
    if (l_internalUseOffset != constants::VALUE_0)
    {
        ...
        {
            m_logger->logMessage(
                "Failed to parse Internal Use Area, error: " +
    ...
    if (l_chassisOffset != constants::VALUE_0)
    {
        ...
        {
            m_logger->logMessage(
                "Failed to parse Chassis Info Area, error: " +
                commonUtility::getErrCodeMsg(l_status.error()));
        }
    }
    ...
    if (l_boardOffset != constants::VALUE_0)
    {
        ...
        {
            m_logger->logMessage(
                "Failed to parse Board Info Area, error: " +
                commonUtility::getErrCodeMsg(l_status.error()));
        }
    }
    ...
    if (l_productOffset != constants::VALUE_0)
    {
        ...
        {
            m_logger->logMessage(
                "Failed to parse Product Info Area, error: " +
                commonUtility::getErrCodeMsg(l_status.error()));
        }
    }
    ...
    if (l_multirecordOffset != constants::VALUE_0)
    {
        ...
        {
            m_logger->logMessage(
                "Failed to parse MultiRecord Area, error: " +
                commonUtility::getErrCodeMsg(l_status.error()));
        }
    }
