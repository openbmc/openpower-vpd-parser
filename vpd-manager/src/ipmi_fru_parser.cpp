    ...
    IpmiFruParser::processCommonHeader() noexcept
{
    ...
    // Helper: convert an IpmiVpdAreaIndex enumerator to size_t once,
    // used both as the result-array subscript and as the Common Header
    // byte index (the spec places each area's offset byte at the same
    // position as the area's enum value).
    constexpr auto toIdx = [](types::IpmiVpdAreaIndex i_area) constexpr {
        return static_cast<size_t>(i_area);
    };

    // l_result is zero-initialised, so COMMON_HEADER's byte offset (0) is
    // already correct.  The remaining areas are read from the Common Header:
    // the spec places each area's offset byte at the same position as the
    // area's enum ordinal, so a single loop covers all of them.
    IpmiFruParser::AreaByteOffsets l_result{};
    for (size_t l_i = toIdx(types::IpmiVpdAreaIndex::INTERNAL_USE_AREA);
         l_i < toIdx(types::IpmiVpdAreaIndex::SIZE); ++l_i)
    {
        l_result[l_i] =
            static_cast<size_t>(m_vpdVector[l_i]) * AREA_OFFSET_MULTIPLIER;
    }
    ...
    // Store the Common Header area map with the format-version byte.
    {
        ...
        m_fruMap[toIdx(types::IpmiVpdAreaIndex::COMMON_HEADER)] =
            std::move(l_hdrMap);
    }
