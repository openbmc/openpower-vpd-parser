                                              ...
                                              types::IPZVpdMap& io_dstVpdMap)
{
         ...
         m_backupAndRestoreCfgJsonObj["backupMap"])
    {
        ...
        if (!extractAndFindRecordInMap(
                l_aRecordKwInfo, io_srcVpdMap, io_dstVpdMap,
        ...
        std::cout << l_srcRecordName << ":" << l_srcKeywordName << ":"
                  << l_dstRecordName << ":" << l_dstKeywordName << std::endl;

        bool l_isPelRequired = l_aRecordKwInfo.value("isPelRequired", false);

        auto [l_srcBinaryValue, l_srcStrValue] =
            getBinaryAndStrKwValue(l_srcRecordName, l_srcKeywordName,
                                   io_srcVpdMap, l_srcServiceName, l_errCode);
        std::cout << "src value:" << l_srcStrValue << ":" << std::endl;
        ...
        if (l_srcBinaryValue != l_dstBinaryValue)
        {
            ...
            {
                // Update keyword's value on hardware and in io_dstVpdMap if not
                // empty
            ...
            {
                // Update keyword's value on hardware and in io_srcVpdMap if not
                // empty
