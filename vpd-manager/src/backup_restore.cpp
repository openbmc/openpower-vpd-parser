...
bool BackupAndRestore::extractAndValidateRecordDetails(
    const auto& i_aRecordKwInfo,
    std::tuple<std::string&, std::string&, std::string&, std::string&,
               types::BinaryVector&>
        o_srcDstRecordKeywordInfo,
        ...
        const std::string& i_serviceName) const noexcept
{
    try
    {
        if (i_recordName.empty() || i_keywordName.empty() ||
            i_serviceName.empty())
        {
        ...
        {
            uint16_t l_errCode{0};
            l_strValue = vpdSpecificUtility::getKwVal(i_vpdMap.at(i_recordName),
                                              ...
                                              types::IPZVpdMap& io_dstVpdMap)
{
         ...
         m_backupAndRestoreCfgJsonObj["backupMap"])
    {
        ...
        if (!extractAndValidateRecordDetails(
                l_aRecordKwInfo,
