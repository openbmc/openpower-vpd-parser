    ...
    const size_t i_maxThreadsPerChassis) noexcept
{
    ...
    try
    {
        ...
        const auto l_fruThread = [this](std::string i_chassisEeepromPath,
                                        std::shared_ptr<FruThreadContext>
                                            i_fruThreadContext) {
            uint16_t l_errCode = 0;
            Worker l_worker;
            ...
            while (true)
            {
                ...
                m_logger->logMessage(
                    std::format("Completed VPD collection for EEPROM [{}]. "
                                "Present: {}, collection status: {}, Error: {}",
                                l_fruPath, l_isPresent, l_collectionStatus,
