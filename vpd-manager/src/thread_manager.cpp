         ...
         l_chassisToMotherboardEepromMap)
    {
        ...
        try
        {
            ...
            std::thread{[l_eepromPath, l_chassisJson, l_chassisId, this]() {
                ...
                if (l_isPresent && constants::vpdCollectionCompleted == l_collectionStatus)
                {
                        std::lock_guard<std::mutex> lock(m_chassisConfigMutex);
                        m_presentChassisIdToConfig[l_chassisId] =
                            l_chassisJson;
                }
