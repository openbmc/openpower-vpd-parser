...
void GpioEventHandler::handleChangeInGpioPin(const bool& i_isFruPresent)
{
    try
    {
        ...
        else
        {
            ...
            l_worker.deleteFruVpd(l_chassisBasedJsonObj, l_invPath);
        }
    }
    ...
    const std::shared_ptr<boost::asio::steady_timer>& i_timerObj)
{
    ...
    if (m_configManager)
    {
        l_chassisBasedJsonObj = m_configManager->getJsonObj();
    }

    bool l_currentPresencePinValue = jsonUtility::processGpioPresenceTag(
        l_chassisBasedJsonObj, m_fruPath, "pollingRequired", "hotPlugging",
        l_errCode);
