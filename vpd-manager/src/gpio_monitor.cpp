    ...
    try
    {
        ...
        if (i_isFruPresent)
        {
            auto [l_isPresent, l_collectionStatus] = l_worker.collectFruVpd(
                m_fruPath, m_singleChassisConfigJson, l_errCode);
        ...
        else
        {
            const std::string l_invPath =
                jsonUtility::getInventoryObjPathFromJson(
                    m_singleChassisConfigJson, m_fruPath, l_errCode);
    ...
    const std::shared_ptr<boost::asio::steady_timer>& i_timerObj)
{
    ...
    bool l_currentPresencePinValue = jsonUtility::processGpioPresenceTag(
        m_singleChassisConfigJson, m_fruPath, "pollingRequired", "hotPlugging",
        l_errCode);
