    ...
    try
    {
        ...
        if (i_isFruPresent)
        {
            ...
            auto [l_isPresent, l_collectionStatus] = l_worker.collectFruVpd(
                m_fruPath, m_chassisBasedJsonObj, l_errCode);
        ...
        else
        {
            ...
            std::string l_invPath = jsonUtility::getInventoryObjPathFromJson(
                m_chassisBasedJsonObj, m_fruPath, l_errCode);
            ...
            l_worker.deleteFruVpd(m_chassisBasedJsonObj, l_invPath);
        }
    }
    ...
    const std::shared_ptr<boost::asio::steady_timer>& i_timerObj)
{
    ...
    {
        logging::logMessage(
            std::format("Timer wait failed for gpio pin. Error: {}.",
    ...
    bool l_currentPresencePinValue = jsonUtility::processGpioPresenceTag(
        m_chassisBasedJsonObj, m_fruPath, "pollingRequired", "hotPlugging",
        l_errCode);

    if (l_errCode && l_errCode != error_code::DEVICE_NOT_PRESENT)
    {
        logging::logMessage(std::format(
            "processGpioPresenceTag returned false for FRU [{}]. Error: {}.",
            m_fruPath, commonUtility::getErrCodeMsg(l_errCode)));
    }
    ...
    const std::shared_ptr<boost::asio::io_context>& i_ioContext)
{
    ...
    {
        logging::logMessage(
            "Error: Config manager is not initialized, can't run setEventHandlerForGpioPresence.");
        return;
    }

    m_chassisBasedJsonObj = m_configManager->getJsonObj(m_fruPath);
    ...
    const std::shared_ptr<ConfigManager>& i_configManager)
{
    ...
    if (i_configManager.get() == nullptr)
    {
        logging::logMessage(
            "Error: Config manager is not initialized, can't process initHandlerForGpio.");
        return;
    }
