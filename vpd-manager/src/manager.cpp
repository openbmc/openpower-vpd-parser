...
Manager::Manager(
    ...
    const std::shared_ptr<sdbusplus::asio::dbus_interface>& commoniFace,
    const std::shared_ptr<sdbusplus::asio::connection>& asioConnection) :
    m_ioContext(ioCon), m_interface(iFace), m_commonInterface(commoniFace),
    m_asioConnection(asioConnection)
{
    ...
    try
    {
        ...
        commoniFace->register_property_rw<std::string>(
        ...
        m_ibmHandler = std::make_shared<IbmHandler>(
            m_worker, m_backupAndRestoreObj, m_interface, m_commonInterface,
            m_ioContext, m_asioConnection);
#else
        m_worker = std::make_shared<Worker>(INVENTORY_JSON_DEFAULT);
        m_commonInterface->set_property(
