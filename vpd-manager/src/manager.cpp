    ...
    const std::string& i_expandedLocationCode)
{
    ...
    if (i_expandedLocationCode.empty() || i_expandedLocationCode[0] != 'U' ||
        i_expandedLocationCode.length() < constants::EXP_LOCATION_CODE_MIN_LENGTH)
    {
    ...
    try
    {
        const auto l_subTree = dbusUtility::getObjectSubTree(
            constants::pimPath, 0, {constants::locationCodeInf});

        for (const auto& [l_objPath, l_serviceMap] : l_subTree)
        {
            if (l_serviceMap.empty())
            {
                continue;
            }

            // Use the first service - all services return the same property value
            const auto& l_service = l_serviceMap.begin()->first;

            // Read the LocationCode property from IBM interface
            const auto l_locationCodeValue = dbusUtility::readDbusProperty(
                l_service, l_objPath, constants::locationCodeInf, "LocationCode");

            if (const auto l_locCode = std::get_if<std::string>(&l_locationCodeValue))
            {
        ...
        {
            m_logger->logMessage(std::format(
                "No FRUs found for expanded location code: {}",
                i_expandedLocationCode));
        }
    }
    catch (const std::exception& l_ex)
