...
namespace vpd
{
Parser::Parser(const std::string& i_vpdFilePath, nlohmann::json i_parsedJson,
    ...
    m_vpdModeBasedFruPath(i_vpdFilePath), m_logger(Logger::getLoggerInstance())
{
    ...
    {
        m_vpdStartOffset =
            jsonUtility::getVPDOffset(m_parsedJson, m_vpdFilePath, l_errorCode);
...
std::shared_ptr<vpd::ParserInterface> Parser::getVpdParserInstance()
{
    ...
    if (l_errCode)
    {
        throw SystemException(static_cast<int>(l_errCode),
                              "Failed to get VPD in vector for: " +
...
bool Parser::compareData(const std::string& i_redundantEepromPath)
{
    ...
    if (typeid(*l_primaryParser.get()) != typeid(*l_redundantParser.get()))
    {
        throw std::runtime_error(
            "VPD type mismatch detected between primary and redundant VPDs.");
    }
