    ...
    const types::WriteVpdParams& i_paramsToWriteData) const noexcept
{
    try
    {
        ...
        const IpzData* l_ipzData = std::get_if<IpzData>(&i_paramsToWriteData);
        if (!l_ipzData)
        {
            return;
        }
        ...
        auto l_checkRecordEccOnFru =
            ...
            if (!l_vpdParserInstance->recordEccCheck(i_paramsToWriteData))
            {
        ...
        if (!l_redundantFruPath.empty())
        {
            ...
            if (auto l_primaryValue = std::get_if<types::BinaryVector>(
                    &l_primaryEepromKeywordValue);
