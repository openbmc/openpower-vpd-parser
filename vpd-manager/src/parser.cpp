...
int Parser::updateVpdKeyword(const types::WriteVpdParams& i_paramsToWriteData)
{
    ...
    try
    {
        ...
        if (!l_inventoryObjPath.empty())
        {
            ...
            // Check for redundant pair of D-bus interface and property for the
            // given record and keyword pair.
            if (const auto identicalPair =
                    dbusUtility::identicalDbusPropertyPairs.find(
                        std::make_pair(l_interfaceName, l_propertyName));
                identicalPair != dbusUtility::identicalDbusPropertyPairs.end())
            {
                l_interfaceMap.emplace(
                    std::get<0>(identicalPair->second),
                    types::PropertyMap{std::make_pair(
                        std::get<1>(identicalPair->second),
                        vpdSpecificUtility::encodeKeyword(
                            commonUtility::toString(l_keywordValue),
                            std::get<2>(identicalPair->second)))});
            }
