...
int Parser::updateVpdKeyword(const types::WriteVpdParams& i_paramsToWriteData)
{
    ...
    try
    {
        ...
        if (SYSTEM_VPD_FILE_PATH == l_fruPath &&
            updateVpdKeywordOnBackupPath(l_fruPath, i_paramsToWriteData) < 0)
        {
            std::string l_errMsg(
...
int Parser::updateVpdKeywordOnBackupPath(
    const std::string& i_fruPath,
    const types::WriteVpdParams& i_paramsToWriteData)
{
    ...
    try
    {
        ...
        else
        {
            ...
            // update.
            std::string l_errMsg(
                "Input parameter type isn't compatible to update keyword's value on backup VPD, for the path[" +
                i_fruPath + "]");
            throw std::runtime_error(l_errMsg);
        }
