    ...
    m_vpdFilePath(vpdFilePath), m_parsedJson(parsedJson)
{
    ...
    m_effectiveFruPath = m_vpdFilePath;
    ...
    commonUtility::getEffectiveFruPath(m_vpdCollectionMode, m_effectiveFruPath,
                                       l_errorCode);
    ...
    if (!std::filesystem::exists(m_effectiveFruPath, l_errCode))
    {
        std::string l_message{"Parser object creation failed, file [" +
                              m_effectiveFruPath + "] doesn't exists."};
...
std::shared_ptr<vpd::ParserInterface> Parser::getVpdParserInstance()
{
    ...
    vpdSpecificUtility::getVpdDataInVector(m_effectiveFruPath, m_vpdVector,
    ...
    std::shared_ptr<vpd::ParserInterface> l_parser = ParserFactory::getParser(
        m_vpdVector, m_effectiveFruPath, m_vpdStartOffset);
