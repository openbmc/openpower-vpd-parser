...
void Listener::registerHostStateChangeCallback() noexcept
{
    try
    {
    ...
    sdbusplus::message_t& i_msg) const noexcept
{
    ...
    catch (const std::exception& l_ex)
    {
        ...
        EventLogger::createSyncPel(
            types::ErrorType::InternalFailure, types::SeverityType::Informational,
            __FILE__, __FUNCTION__, 0,
            "Host state change callback update failed, reason: " +
    ...
    sdbusplus::message_t& i_msg) const noexcept
{
    ...
    catch (const std::exception& l_ex)
    {
        ...
        EventLogger::createSyncPel(
            types::ErrorType::InternalFailure, types::SeverityType::Informational,
            __FILE__, __FUNCTION__, 0,
            "AssetTag change callback update failed, reason: " +
                std::string(l_ex.what()),
            std::nullopt, std::nullopt, std::nullopt, std::nullopt);
    }
}
