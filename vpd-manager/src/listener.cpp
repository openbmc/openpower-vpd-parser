    ...
    const std::string& i_correlatedPropJsonFile) noexcept
{
    try
    {
        ...
        for (const auto& l_service : l_serviceObjectList.items())
        {
            ...
            std::for_each(l_serviceObject.items().begin(),
                          l_serviceObject.items().end(),
