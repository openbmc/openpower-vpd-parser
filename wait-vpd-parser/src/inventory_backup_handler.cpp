...
>>>>>>> 80262e7 (Implement reading a property value from a PIM serialised backup file)
{
    ...
    try
    {
        ...
        // Manager ordinal as persisted by cereal for
        // PhysicalContextType::Manager
        constexpr int l_managerOrdinal = 1;
    ...
    const std::filesystem::path& i_entryPath) const noexcept
{
    ...
    return m_skipInterfaceSet.count(i_entryPath.filename().string()) != 0 &&
           m_bmcPaths.count(i_entryPath.parent_path().string()) != 0;
}
...
void InventoryBackupHandler::moveDirectory(
    const std::filesystem::path& i_srcPath,
    const std::filesystem::path& i_dstPath,
    std::vector<std::filesystem::path>& o_failedPaths) const
{
    std::error_code l_ec;
    if (!std::filesystem::is_directory(i_srcPath, l_ec))
    {
        if (l_ec)
        {
            m_logger->logMessage(std::format(
                "Failed to check if directory entry {}, is directory. Error : {}.",
                i_srcPath.string(), l_ec.message()));
        }
        return;
    }

    for (const auto& l_entry : std::filesystem::directory_iterator(i_srcPath))
    {
        l_ec.clear();
...
bool InventoryBackupHandler::moveFiles(
