...
int FruVpdCollectionManager::triggerFruVpdCollectionAndCheckStatus()
{
    ...
    try
    {
        ...
        // I/O context has been stopped, check if we have successfully triggered
        // FRU VPD collection
        if (m_collectAllFruVpdTriggered)
        {
            // update return value as per collection status(0 for collection
            // done, 1 for collection not done)
            l_rc = !m_collectionDone;
        }
        else
        {
            // failed to trigger FRU VPD collection, throw exception in order to
            // fail this service
            throw vpd::DbusException("Failed to trigger FRU VPD collection");
        }
    }
    ...
    boost::system::error_code i_ec, sdbusplus::message_t& i_msg) noexcept
{
    try
    {
        ...
        else
        {
            ...
            i_msg.read(m_collectAllFruVpdTriggered);
    ...
    {
        m_logger->logMessage("Error processing collect all FRU VPD callback: " +
                             std::string(l_ex.what()));
