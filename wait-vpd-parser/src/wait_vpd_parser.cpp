...
/**
 * @brief API to trigger FRU VPD collection
 *
 * This API triggers FRU VPD collection by calling Dbus API "CollectAllFRUVPD"
 * exposed by vpd-manager
 *
 * @return - On success returns true, otherwise returns false
 */
inline bool collectFruVpd() noexcept
{
    bool l_rc{true};
    try
    {
    ...
    {
        auto l_logger = vpd::Logger::getLoggerInstance();
        l_logger->logMessage("Failed to trigger FRU VPD collection. Error: " +
                             std::string(l_ex.what()));
        l_rc = false;
    }
    return l_rc;
...
int main(int argc, char** argv)
{
    ...
    return collectFruVpd() ? checkVpdCollectionStatus(l_retryLimit,
