 ...
 *
 * @param[in] i_gpioName Name of the GPIO line
 * @return 0 or 1 on success, -1 on failure. All exceptions are caught
 *         locally.
 */
inline int readGpioValue(const std::string& i_gpioName) noexcept
{
    try
    {
        gpiod::line l_line = gpiod::find_line(i_gpioName);
        if (!l_line)
        {
        ...
        l_line.request({consumerName, gpiod::line_request::DIRECTION_INPUT, 0});

        const auto l_value = l_line.get_value();
        l_line.release();
...
int main()
{
    ...
    // Read the GPIO value
    const int l_pgood = pgood_chassis_check::readGpioValue(l_gpioName);
