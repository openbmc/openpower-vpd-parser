...
{
    /** @todo
     *  1. Call gpiod::find_line(i_gpioName) to locate the named GPIO line
     ...
     *  off.
     *  2. Call i_line.request() with DIRECTION_INPUT and consumerName.
     *  Catch any exception, log the error via lg2, and return false so the
     *  caller defaults to chassis off.
     *  3. Call i_line.get_value() to read the GPIO pin level (0 or 1).
