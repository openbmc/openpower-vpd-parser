 ...
 *         locally.
 */
int publishChassisPowerState(types::PowerStateIface::State state) noexcept
{
    try
    {
...
types::BmcPosition readBmcPosition() noexcept
{
    try
    {
        ...
        return (bmcPosition == 0) ? types::BmcPosition::POSITION_0
                                  : types::BmcPosition::POSITION_1;
    }
    catch (const std::exception& ex)
    {
        lg2::error(
            "pgood-chassis-check: exception while trying to read BMC position from file."
            "{ERR}. Returning BMC position as default value, 0",
            "ERR", ex.what());
    }
    return types::BmcPosition::DEFAULT;
}
...
int main()
{
    try
    {
        ...
        if (pgood == pgood_chassis_check::types::GpioValue::INVALID_VALUE)
        {
    ...
    {
        lg2::error("pgood-chassis-check: failed"
                   "{ERR}",
                   "ERR", ex.what());
