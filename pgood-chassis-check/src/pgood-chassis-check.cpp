...
bool setChassisPowerState(PowerStateIface::State i_state) noexcept
{
    try
    {
        ...
        auto l_method =
            l_bus.new_method_call(pimServiceName, systemInvPath,
                                  "org.freedesktop.DBus.Properties", "Set");
        l_method.append(PowerStateIface::interface,
                        PowerStateIface::property_names::power_state,
                        std::variant<std::string>{l_stateStr});
        l_bus.call(l_method);

        lg2::info("pgood-chassis-check: set PowerState='{STATE}' on D-Bus",
                  "STATE", l_stateStr);
        return true;
    }
    catch (const std::exception& l_ex)
    {
        lg2::error("pgood-chassis-check: failed to set PowerState on "
                   "D-Bus: {ERR}",
                   "ERR", l_ex.what());
        return false;
    }
