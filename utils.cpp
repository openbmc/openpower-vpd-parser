...
string udevToGenericPath(string& file)
{
    ...
    if (file.find("i2c") != string::npos)
    {
        ...
        file = std::regex_replace(file, udevPattern, i2cPathPrefix);
    }
    ...
    else if (file.find("spi") != string::npos)
    {
        ...
        file = std::regex_replace(file, udevPattern, spiPathPrefix);
    }
    else
    {
        cerr << "\n The given EEPROM path is not valid. It's neither I2C nor "
                "SPI path. Skipping VPD collection for the given path."
             << endl;
        exit(EXIT_SUCCESS);
    }
    return file;
