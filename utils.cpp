...
MapperResponse
    getObjectSubtreeForInterfaces(const std::string& root, const uint8_t& depth,
                                  const std::vector<std::string>& interfaces)
{
    ...
    mapperCall.append((int32_t)depth);
    ...
    try
    {
        ...
        if (result.empty())
        {
            log<level::ERR>("Invalid response from mapper for GetSubTree");
        }
    }
    catch (const sdbusplus::exception::SdBusError& e)
    {
