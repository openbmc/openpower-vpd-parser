...
void VpdTool::debugger(json output) { cout << output.dump(4) << '\n'; }
...
void VpdTool::addFruTypeAndLocation(json exIntf, const string &object,
  ...
  for (auto intf : exIntf.items()) {
    if ((intf.key().find("Item") != string::npos) && (intf.value().is_null())) {
  ...
  try {
    ...
    makeDBusCall(object, LOCATION_CODE_IF, LOCATION_CODE_PROP).read(response);
...
json VpdTool::interfaceDecider(json &itemEEPROM) {
  ...
  if (itemEEPROM.value("inherit", true)) {
    ...
    for (auto ex : itemEEPROM["extraInterfaces"].items()) {
      if (!(ex.value().is_null())) {
        exIntfCheck = true;
        getExtraInterfaceProperties(itemEEPROM.at("inventoryPath"), ex.key(),
