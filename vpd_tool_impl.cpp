...
int VpdTool::fixSystemVPD()
{
    ...
    Binary vpdVector{};
    auto vpdDataMap = getVPDInMap(
        constants::systemVpdFilePath, js,
        static_cast<std::string>(constants::SYSTEM_OBJECT), vpdVector);

    Parsed vpdMap;
    if (auto pVal = std::get_if<Store>(&vpdDataMap))
    {
        vpdMap = pVal->getVpdMap();
    }
    else
    {
        std::string err = std::string(constants::systemVpdFilePath) +
