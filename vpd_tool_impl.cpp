...
int VpdTool::fixSystemVPD()
{
    ...
    int num = 0;
    vector<tuple<int, string, string, string, string, string>> recKwData;
    ...
    for (const auto& recordKw : svpdKwdMap)
    {
        for (const auto& keyword : recordKw.second)
        {
            ...
            const auto busValue = readDBusProperty<variant<Binary>>(
                constants::pimIntf,
                "/xyz/openbmc_project/inventory/system/chassis/motherboard",
                constants::ipzVpdInf + record, keyword);
            ...
            const auto busData = get_if<Binary>(&busValue);
    ...
    do
    {
        ...
        ifstream inventoryJson(INVENTORY_JSON_SYM_LINK, ios::binary);
        ...
        else if (option == VpdTool::MORE_OPTIONS)
        {
            ...
            for (const auto& data : recKwData)
            {
                int maxRetry = 3; // 3 Maximum retries.
                do
                {
                    cout << "\n|=========================================="
                            "========"
