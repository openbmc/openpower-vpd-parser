...
class VpdTool
{
  private:
     ...
     */
    int dataFileToBinary(
        openpower::vpd::Binary& data, const uint32_t offset,
    ...
    int stringToFile(const std::string& input,
                     const enum openpower::vpd::constants::FileType& fileType,
  ...
  public:
     ...
     * console if fileType is UNKNOWN. Supported file types are binary and text.
     ...
     */
    int updateKeyword(const uint32_t offset,
                      const enum openpower::vpd::constants::FileType& fileType);
