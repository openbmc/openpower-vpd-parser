...
void EditorImpl::checkRecordForKwd()
{
    ...
    auto iterator = (thisRecord.recData).cbegin() + skipBeg + lengths::RECORD_NAME;
    ...
    while (iterator < end)
    {
        ...
        if (thisRecord.recKWd == kw)
        {
            ...
            std::size_t kwdOffset =
                std::distance(start, iterator);
            thisRecord.relKwOffset = std::distance(thisRecord.recData.cbegin(),
                                                   iterator);
                    ...
                    nlohmann::json::value_t::object)
                {
                          ...
                          thisRecord.recKWd)))
                    {
                        ...
                        makeDbusCall<std::string>(
                            ...
                            encodeKeyword(
                                kwdData,
                                ei_PropertyList.value().value("encoding", "")));
                    }
                }
            }
        }
