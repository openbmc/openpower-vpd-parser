                                    ...
                                    std::string recName)
{
    ...
    while (iterator < end)
    {
        ...
        else
        {
            // Jump the record
            std::advance(iterator, lengths::RECORD_NAME + sizeof(RecordType)+
                                sizeof(RecordSize) + sizeof(RecordLength) +
...
void Editor::updateData(Binary kwdData, std::size_t dataLength, Binary::iterator iterator)
{
...
void Editor::checkRecordForKwd(RecordOffset recOffset, std::string kwdName, Binary kwdData)
{
    // Jump to record name
