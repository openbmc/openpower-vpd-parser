...
void Editor::writeFile()
{
    ...
    auto iteratorEnd = vpd.cend();

    std::copy(iteratorStart, iteratorEnd,
              std::ostream_iterator<uint8_t>(updatedVPD));
}
                           ...
                           Binary kwdData)
{
    ...
    openpower::vpd::modify::createWriteEccForThisRecord(vpd, iterator);
