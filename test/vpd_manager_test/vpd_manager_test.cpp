...
class vpdManagerTest : public ::testing::Test
{
  ...
  public:
    ...
    void processJson(const nlohmann::json& parsedJson);
};

void vpdManagerTest::processJson(const nlohmann::json& parsedJson)
{
    if (parsedJson.find("frus") == parsedJson.end())
    {
...
/*
TEST_F(vpdManagerTest, InvalidKwdTest)
{
    std::string result{};
    try
    {
...
TEST_F(vpdManagerTest, InvalidHeader)
{
    std::string result{};
    try
    {
...
TEST_F(vpdManagerTest, InvalidRecordTest)
{
    std::string result{};
    try
    {
...
TEST_F(vpdManagerTest, DataLengthLessThanKwdSize)
{
...
TEST_F(vpdManagerTest, Allvalid_kwdUpdatedSuccessfully)
{
    // length 12 is used because the kwd we are updating is of length 12
    Binary dataToUodate{'M', 'O', 'D', 'I', 'F', 'Y',
                        'D', 'A', 'T', 'A', 'O', 'K'};
...
*/
TEST_F(vpdManagerTest, getExpandedLocationCode_validInput)
{
