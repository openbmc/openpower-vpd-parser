...
TEST_F(vpdManagerReaderTest, getExpandedLocationCode_Valid)
{
    ...
    MockUtilCalls uCalls;
    EXPECT_CALL(uCalls, readBusProperty("test", "test", "test"))
        .Times(1)
...
TEST_F(vpdManagerReaderTest, getFrusAtLocation_Invalid)
{
    ...
    ReaderImpl read(uCalls);
    EXPECT_ANY_THROW({
        read.getExpandedLocationCode(unexpandedLocationCode, nodeNumber,
                                     fruLocationCode);
    });
...
TEST_F(vpdManagerReaderTest, getCollapsedLocationCode_invalid)
{
    ...
    EXPECT_ANY_THROW({ res = read.getCollapsedLocationCode(locationCode); });
    ...
    EXPECT_CALL(uCalls, readBusProperty("testFC", "testFC", "testFC"))
        .Times(1)
        .WillOnce(
...
TEST_F(vpdManagerReaderTest, getCollapsedLocationCode_Valid_FC)
{
    ...
    ReaderImpl read(uCalls);
    std::tuple<LocationCode, Node> res;
    res = read.getCollapsedLocationCode(validLocationCode);

    LocationCode locationCode = std::get<0>(res);
    Node node = std::get<1>(res);

    EXPECT_EQ(locationCode, "Ufcs-P0");
    EXPECT_EQ(node, 1);

    validLocationCode = "U78DA.ND1.1234567";
    res = read.getCollapsedLocationCode(validLocationCode);
    locationCode = std::get<0>(res);
    node = std::get<1>(res);

    EXPECT_EQ(locationCode, "Ufcs");
    EXPECT_EQ(node, 1);
}

TEST_F(vpdManagerReaderTest, getCollapsedLocationCode_Valid_TM)
{
    ...
    EXPECT_CALL(uCalls, readBusProperty("testTM", "testTM", "testTM"))
        .Times(1)
        .WillOnce(
