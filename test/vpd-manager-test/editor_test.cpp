...
TEST_F(vpdManagerEditorTest, InvalidFile)
{
    ...
    {
        // Invalid kwd name
        EditorImpl edit("test.dat", jsonFile, "VINI", "SN",
                        std::move(emptyVpdFile));
...
TEST_F(vpdManagerEditorTest, InvalidHeader)
{
    ...
    {
        // the path is dummy
        EditorImpl edit("pathFile", jsonFile, "VINI", "SN", std::move(vpd));
        edit.updateKeyword(dataToUodate);
    }
    catch (const std::exception& e)
    {
...
TEST_F(vpdManagerEditorTest, InvalidRecordName)
{
    ...
    try
    {
        ...
        EditorImpl edit("test.dat", jsonFile, "VIN", "SN", std::move(vpd));
        edit.updateKeyword(dataToUodate);
    }
    catch (const std::exception& e)
    {
...
TEST_F(vpdManagerEditorTest, InvalidKWdName)
{
    ...
    {
        // All valid data
        EditorImpl edit("test.dat", jsonFile, "VINI", "Sn", std::move(vpd));
        edit.updateKeyword(dataToUodate);
    }
