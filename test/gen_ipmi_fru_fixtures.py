...
def build_chassis_area(chassis_type: int, part: str, serial: str,
                       customs=None) -> bytes:
...
def build_board_area(lang: int, mfg_minutes: int, manufacturer: str,
                     product: str, serial: str, part: str, fru_file_id: str,
                     customs=None) -> bytes:
...
def build_product_area(lang: int, manufacturer: str, product_name: str,
                       part_model: str, version: str, serial: str,
                       asset_tag: str, fru_file_id: str, customs=None) -> bytes:
    body = bytes([0x01])
    body += bytes([0x00])
    body += bytes([lang])
    body += ascii8(manufacturer)
...
def make_all_areas():
    ...
    internal = build_internal_area(internal_payload)  # 12 bytes (1 ver + 11 data)
...
def make_bad_format_version():
    ...
    prod = build_product_area(
        lang=0, manufacturer="X", product_name="Y",
        part_model="Z", version="1", serial="S", asset_tag="A", fru_file_id="",
    )
...
def make_bad_board_checksum():
    board = build_board_area(
        lang=0, mfg_minutes=0,
        manufacturer="IBM", product="Board", serial="SN1", part="PN1", fru_file_id="",
    )
    hdr = build_common_header({"board": 8})
...
def make_field_overrun():
    prod = build_product_area(
        lang=0, manufacturer="IBM", product_name="Drive",
        part_model="PM9", version="1", serial="SN001", asset_tag="A", fru_file_id="",
    )
    hdr = build_common_header({"product": 8})
    image = bytearray(hdr + prod)
    ...
    new_cs = zero_checksum(bytes(image[prod_start:prod_start + prod_len_bytes - 1]))
