...
namespace ConfigManagerJsonValidator
{
...
class Manager
{
  public:
    ...
    static std::unique_ptr<vpd::ConfigManager> createConfigManager(
        const std::string& jsonPath)
    {
        // Create the passkey (only Manager can do this)
        vpd::ConfigManager::ManagerPassKey key;

        // Create and return ConfigManager instance
        return std::make_unique<vpd::ConfigManager>(key, jsonPath);
    }
};
...
void expectConfigManagerCreationSuccess(const nlohmann::json& json)
{
    ...
    EXPECT_NO_THROW({
        auto configMgr = Manager::createConfigManager(testJsonPath);
        EXPECT_NE(configMgr, nullptr);
    });
}

// Helper to attempt ConfigManager creation and expect JsonException
void expectConfigManagerCreationFailure(
    const nlohmann::json& json, const std::string& expectedErrorSubstring)
{
    writeJsonToFile(json);
    EXPECT_THROW(
        {
...
TEST_F(ConfigManagerValidationTest, FrusNotObject_ThrowsException)
{
    ...
    expectConfigManagerCreationFailure(json,
                                       "'frus' section must be an object");
}
