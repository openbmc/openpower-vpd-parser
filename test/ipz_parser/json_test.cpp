...
TEST(verifyInventoryJSON, inventoryGoodPath)
{
    ...
    int fd = access("/usr/share/vpd/vpd_inventory.json", F_OK);
    if (fd == -1)
    {
        ...
        "/usr/share/vpd/vpd_inventory.json"); // INVENTORY_JSON);
    {
        ...
        {
            // TEST3: Check for all the FRU's path is defined in the inventory
            // JSON
            if (js["frus"].find(eachFru) == js["frus"].end())
            {
                throw std::runtime_error(
                    "Device path missing in inventory JSON");
            }
        ...
        // TEST9: Read "recordName" & "keywordName".This is suppose to be VINI &
        // PN respectively.
        string rec = js["commonInterfaces"]
                       ["xyz.openbmc_project.Inventory.Decorator.Asset"]
                       ["PartNumber"]["recordName"];
        string kw = js["commonInterfaces"]
                      ["xyz.openbmc_project.Inventory.Decorator.Asset"]
        ...
        // TES11: Read "recordName" & "keywordName".This is suppose to be VINI &
        // SN respectively.
        rec = js["commonInterfaces"]
                ["xyz.openbmc_project.Inventory.Decorator.Asset"]
                ["SerialNumber"]["recordName"];
        kw = js["commonInterfaces"]
        ...
        // TES14: Read "recordName" & "keywordName".This is suppose to be VINI &
        // DR respectively.
