...
{
    auto vpdPtr = vpd.cbegin();
    std::string failedRecordData = byteVecToHexString(
        vpd.begin() + offsets::VHDR_RECORD, lengths::VHDR_RECORD_LENGTH);
    std::string failedEccData = byteVecToHexString(
        vpd.begin() + offsets::VHDR_ECC, lengths::VHDR_ECC_LENGTH);
    auto l_status =
...
void Impl::recordEccCheck(Binary::const_iterator iterator) const
{
    ...
    std::string recordName(itrToRecName, itrToRecName + lengths::RECORD_NAME);
