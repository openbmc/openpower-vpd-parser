...
int Impl::vhdrEccCheck() const
{
    ...
    uint32_t l_status = vpdecc_check_data(
                          (uint8_t*)& vpdPtr[vhdrOffset],
                          vhdrLength,
                          &vpdPtr[vEccOffset],
                          vEccLength);
...
int Impl::vtocEccCheck() const
{
    ...
    uint32_t l_status = vpdecc_check_data(
                          (uint8_t*)& vpdPtr[vtocOffset],
                          vtocLength,
                          &vpdPtr[vtocECCOffset],
                          vtocECCLength);
    if ( l_status != VPD_ECC_OK)
    {
        rc = -1;
...
void Impl::getOffsetAndLengthForECCAndRecord(
...
int Impl::recordEccCheck(std::string recordName) const
{
    ...
    getOffsetAndLengthForECCAndRecord(recordName, recordOffset, recordLength, EccOffset, EccLength);
