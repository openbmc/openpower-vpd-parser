...
#ifdef IPZ_PARSER
        // Reverse back to RT Kw, in ipz vpd, to Read RT KW
        std::prev(iterator,
                  lengths::RECORD_NAME + lengths::KW_NAME + sizeof(KwSize));
#endif
...
internal::KeywordMap Impl::readKeywords(Binary::const_iterator iterator)
{
    ...
    while (true)
    {
        ...
        std::string isPoundKw(iterator, iterator + 1);
