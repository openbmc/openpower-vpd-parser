...
int main(int argc, char** argv)
{
    ...
    try
    {
            ...
            (arguments.end() != arguments.find("object")))
        {
            ...
            //            ipzparser::ipzImpl p(std::move(vpd));
            //            auto vpdStore = p.run();
            ...
            /*
                        ...
                        {
                            std::cerr << "Unequal number of FRU types and object
               paths " "specified\n"; rc = -1;
                        }
                        else
                        {
                            // Write VPD to FRU inventory
                            for (std::size_t index = 0; index < frus.size();
               ++index)
                            {
                                inventory::write(frus[index], vpdStore,
               objects[index]);
                            }
                        }*/
        }
