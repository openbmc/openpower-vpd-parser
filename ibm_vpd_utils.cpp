...
std::string arrayToHexString(const Vec& vec)
{
    std::string str;
    std::visit(
        [&str](auto&& vec) {
...
std::string getPrintableValue(const Vec& vec)
{
    std::string str{};
    bool printable = true;
    std::visit(
        [&str, &printable](auto&& elem) {
            ...
            if (it != elem.end())
            {
                for (auto itr = it; itr != elem.end(); itr++)
                {
                    if (*itr != 0x00)
                    {
                        str = std::string(elem.begin(), elem.end());
                        printable = false;
                        break;
                    }
                }
