 ...
 * type tag'. 3) For each keyword-value pairs : 3.1) Parse the 2 byte length
 ...
 * inserted in 3.1. 4) Validate the byte before checksum byte is 'small resource
 * type end tag'. 5) Validate the checksum. 6) Validate the 'small resource type
 * last end tag'. 7) Return the keyword-value map.
 */
...
class KeywordVpdParser
{
  private:
    ...
    /**
     * @brief Validate the vendor defined large resource type tag
     */
    void validateVendorDefinedLargeResourceType();
