 ...
 * Use this function to insert the operational status and availability
 * properties into the inventory map. This function first checks if the object
 * in question already has these properties hosted on D-Bus, if the property is
 * already there, it is not modified, hence the name "one time". If the property
 * is not already present, it will be added to the map with a suitable default
 * value (true for Operational and false for Available)
 *
                                 ...
                                 inventory::InterfaceMap& interfaces)
{
    ...
    prop = bus.new_method_call("xyz.openbmc_project.Inventory.Manager",
                               objectPath.c_str(),
                               "org.freedesktop.DBus.Properties", "Get");
    prop.append("xyz.openbmc_project.State.Decorator.Availability");
    prop.append("Available");
    try
    {
        auto result = bus.call(prop);
    }
    catch (const sdbusplus::exception::SdBusError& e)
    {
        // Treat as property unavailable
        inventory::PropertyMap prop;
        prop.emplace("Available", false);
        interfaces.emplace("xyz.openbmc_project.State.Decorator.Availability",
                           move(prop));
    }
}
