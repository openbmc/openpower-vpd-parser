        ...
        if (it != vpdMap.end())
        {
            ...
            for (const auto& keyword : kwdListForRecord)
            {
                ...
                if (iterator != kwdValMap.end())
                {
                    ...
                    // never be blank and 0x00 should be treated as no value.
                    bool nonEmptyBusValue =
                        (recordName == "UTIL")
                            ? ((keyword == "D0") &&
                               (!busValue.empty() && (Byte)busValue[0] != 0x00))
                            : ((busValue.find_first_not_of(' ') !=
                                string::npos));

                    bool nonEmptyKWdValue =
                        (recordName == "UTIL")
                            ? ((keyword == "D0") &&
                               (!kwdValue.empty() && (Byte)kwdValue[0] != 0x00))
                            : ((kwdValue.find_first_not_of(' ') !=
                                string::npos));
