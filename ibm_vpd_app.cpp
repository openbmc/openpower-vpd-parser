...
void configGPIO(nlohmann::json& json)
{
    ...
    uint8_t polarity, pinNo, togglePinNo = 0;
    ...
    for (auto eachFRU : json["frus"].items())
    {
        for (auto eachInventory : eachFRU.value())
        {
            ...
            if (toggleGPIO)
            {
                ...
                //TODO: correct gpio chip no
                ::gpiod::chip chip("gpiochip0");
                auto line = chip.get_line(pinNo);
                ...
                if( !(gpioData ^ polarity ))
                {
                    ...
                    {
                        if( preAction.key().find("pinNum") !=  string::npos)
                        {
                            togglePinNo =  preAction.value();
                        }
                    ...
                    auto line2 = chip.get_line(togglePinNo);
