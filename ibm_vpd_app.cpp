...
template <typename T> static void call_func(const T& map, string preIntrStr, inventory::InterfaceMap& interfaces)
{
                               ...
                               const T& ibmVpdMap)
{
    for (const auto& ifs : js.items())
    {
        ...
        for (const auto& itr : ifs.value().items())
        {
            ...
            if constexpr (std::is_same<T,Parsed>::value)    
	    {
			if (!rec.empty() && !kw.empty() && ibmVpdMap.at(rec).count(kw) && ibmVpdMap.count(rec))
			{
    ...
    try
    {
        ...
        switch (type)
        {
            case IPZ_VPD:
            {
                ...
                populateDbus(vpdMap, js, objectPath, file,
                             preIntrStr);
            }
            break;
	        ...
	        // Creating Keyword Vpd Parser Object
                ...
                populateDbus(kwValMap, js, objectPath, file,
                             preIntrStr);
            }
            break;
            default:
