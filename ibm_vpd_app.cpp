...
static const deviceTreeMap deviceTreeSystemTypeMap = {{SYSTEM_2U, "fw_dt1"},
                                                      {SYSTEM_4U, "fw_dt2"}};
...
static vector<string> executeCmd(const char* path,
                                 uint8_t argsNum, ...)
{
...
void setEnvAndReboot(const string& key, const string& value)
{
    ...
    executeCmd("/sbin/fw_setenv", 2, key, value);
...
void setDevTreeEnv(const string& systemType)
{
    ...
    if (deviceTreeSystemTypeMap.find(systemType) != deviceTreeSystemTypeMap.end())
    {
    ...
    vector<string> output = executeCmd("/sbin/fw_printenv", 0);
    ...
    if (!envVarFound)
    {
        ...
        setEnvAndReboot("devTree",newDeviceTree);
    }
}
