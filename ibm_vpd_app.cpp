...
/*
 * @brief This API retrieves keyword values from map  
 *
 * @param[in] backupVpdMap - IPZ vpd map of the backup VPD.
 ...
 * */
bool getKwValue(Parsed& backupVpdMap,
                            ...
                            std::string& backupValue)
{
    ...
    if (recItr != backupVpdMap.end())
    {
        ...
        {
        	string errorMsg = backupVpdKwName + " Keyword not found in "
                                             "the backup VPD file ";
            ...
            for (const auto& keywordInfo : kwdListForRecord)
            {
                ...
                if (iterator != kwdValMap.end())
                {
                    ...
                    if (backupDataInBinary != defaultValue)
                    {
                        ...
                        // If the backup is on the cache we need to copy the backup
                        // data to the VPD map to ensure there is no mimatch b/n them.
                        // So if backup data is not default, then irrespective of primary
                        // data(default or other than backup), copy the backup data to
                        // vpd map as we don't need to change the backup data in either
                        // case in the process of restoring system vpd.
