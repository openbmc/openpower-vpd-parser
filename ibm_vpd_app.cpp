...
tuple<uint16_t, uint16_t> checkSNandFNoffset(const string& filePath)
{
    ...
    if (filePath.find("i2c") != string::npos)
    {
        jsonName = "test"; // filePath.substr(filePath.find("i2c"));
    }
    ...
    if (!offsetJSon)
    {
        std::cout << "json file not found for " << filePath << std::endl;
        return offset;
    }
    ...
    {
        std::cout << "SN " << js["snOffset"] << " FN " << js["fnOffset"]
                  << std::endl;
