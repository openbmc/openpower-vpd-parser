...
Parsed processParentFruVpd(nlohmann::json& js, const string& parentFruVpdPath)
{
    ...
    for (const auto& offset : offsets)
    {
        ...
        advance(iterator,offset + sizeof(RecordId) + sizeof(RecordSize)+lengths::KW_NAME + sizeof(KwSize));
            ...
            if constexpr (std::is_same<T, Parsed>::value)
            {
                ...
                if (item.find("type") != item.end())
                {
                    if( item["type"] == "moduleOnly")
                    {
                        ...
                        populateInterfaces(js["commonInterfaces"], interfaces, moduleCiVpdMap,
                                                       isSystemVpd);
                    }
                }
            }
