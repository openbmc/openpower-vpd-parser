...
static auto expandLocationCode(const string &unexpanded, const Parsed &vpdMap,
  ...
  try {
    ...
    std::cerr << "Failed to expand location code with exception: " << e.what()
              << "\n";
  }
  return expanded;
}
    ...
    for (const auto &itr : ifs.value().items()) {
      ...
      if (itr.value().is_boolean()) {
        ...
        if constexpr (std::is_same<T, Parsed>::value) {
          if (busProp == "LocationCode" && inf == "com.ibm.ipzvpd.Location") {
            auto prop = expandLocationCode(itr.value().get<string>(), vpdMap,
                                           isSystemVpd);
        ...
        if constexpr (std::is_same<T, Parsed>::value) {
          if (!rec.empty() && !kw.empty() && vpdMap.count(rec) &&
            ...
            auto encoded = encodeKeyword(vpdMap.at(rec).at(kw), encoding);
            props.emplace(busProp, encoded);
          }
        } else if constexpr (std::is_same<T, KeywordVpdMap>::value) {
          if (!kw.empty() && vpdMap.count(kw)) {
            auto prop = string(vpdMap.at(kw).begin(), vpdMap.at(kw).end());
...
inventory::ObjectMap primeInventory(nlohmann::json &jsObject, const T &vpdMap) {
  ...
  for (auto itemFRUS : jsObject["frus"].items()) {
    for (auto itemEEPROM : itemFRUS.value()) {
      ...
      sdbusplus::message::object_path object(itemEEPROM.at("inventoryPath"));
          ...
          if (eI.key() == "com.ibm.ipzvpd.Location") {
            if constexpr (std::is_same<T, Parsed>::value) {
              for (auto &lC : eI.value().items()) {
                auto propVal = expandLocationCode(lC.value().get<string>(),
                                                  vpdMap, isSystemVpd);
      ...
      if constexpr (std::is_same<T, Parsed>::value) {
        ...
        for (const auto &record : vpdMap) {
          populateFruSpecificInterfaces(record.second,
                                        preIntrStr + record.first, interfaces);
        }
      } else if constexpr (std::is_same<T, KeywordVpdMap>::value) {
          ...
          for (const auto &record : item["copyRecords"]) {
            ...
            if (vpdMap.find(recordName) != vpdMap.end()) {
              populateFruSpecificInterfaces(
                  vpdMap.at(recordName), preIntrStr + recordName, interfaces);
            }
          }
        }
      }
