...
#ifdef IPZ_PARSER
int processIbmVpd(int argc, char** argv)
{
...
#else
int processOpenPowerVpd(int argc, char** argv)
{
    int rc = 0;

    args::Args arguments = args::parse(argc, argv);

    if (!args::isArgumentValid(arguments))
    {
        return -1;
    }
    ...
    ParserInterface* parser = ParserFactory::getParser(std::move(vpd));
    // string preIntrStr = parser->getInterfaceName();
    ...
    auto vpdStore = std::move(std::get<Store>(parser->parse()));

    if (arguments.count("dump"))
    {
        vpdStore.dump();
    }

    // Set FRU based on FRU type and object path
    if (arguments.count("fru") && arguments.count("object"))
    {
        ...
        {
            OpenpowerVpdParser* OpenPwrParser =
                dynamic_cast<OpenpowerVpdParser*>(parser);

            if (OpenPwrParser == nullptr)
            {
            ...
            {
                OpenPwrParser->write(frus[index], vpdStore, objects[index]);
            }
        }
    }
    return rc;
...
int main(int argc, char** argv)
{
    ...
    {
#ifdef IPZ_PARSER
        rc = processIbmVpd(argc, argv);
#else
        rc = processOpenPowerVpd(argc, argv);
#endif
    }
    catch (exception& e)
    {
