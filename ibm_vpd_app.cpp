...
/** This API will be called for PCIe If vpd-collection failed
 */
void configGPIOpostAction()
{
    //- read object "post_action", it's Pin, and action set OR reset

    /*for this PCI inventory
    {
        for (const auto& postAction :
             eachInventory["postAction"].items())
        {
            if (postAction.key().find("pin") != string::npos)
            {
                pinName = postAction.value();
            }
            else if (postAction.key().find("enable") != string::npos)
            {
                setOrReset = postAction.value();
            }
        }

        gpiod::line line = gpiod::find_line(pinName);
        line.request({"TogglePin",
                       ::gpiod::line_request::DIRECTION_OUTPUT, 0},
                      0);

        line.set_value(setOrReset);
                    ...
                    const string& kwdName, const Binary& data)
{
    ...
    catch (const sdbusplus::exception::SdBusError& e)
    {
        size_t bufSize = snprintf(
            NULL, 0,
            "VPDManager WriteKeyword api failed for inventory path = %s, "
            "record %s and keyword %s with bus error = %s",
            objectName.c_str(), recName.c_str(), kwdName.c_str(), e.what());

        // 1 for numm character at end
        char* what = new char[bufSize + 1];
        snprintf(what, bufSize + 1,
                 "VPDManager WriteKeyword api failed for inventory path = %s, "
                 "record %s and keyword %s with bus error = %s",
                 objectName.c_str(), recName.c_str(), kwdName.c_str(),
                 e.what());
                         ...
                         const string& filePath)
{
    ...
    // call only when vpdCollection failed for PCIe card
    // configGPIOpostAction();
