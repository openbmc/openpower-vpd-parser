 ...
 */
static void getEvenOddCount(size_t& num, size_t& numOfEvenNumbrs,
...
/**
 * @brief Get the SDRAM parameters from the SPD for Symmetric
 *        Module & Asymmetric Even Rank Module
 *
                                              ...
                                              size_t& sdramDnstyPerDie)
{
    ...
    if (tmp > SPD_JEDEC_DDR5_SDRAM_IO_WIDTH_RESERVED_BITS)
    {
        cerr << "Bad data in vpd byte 10 i.e " << tmp
             << ". So Can't calculate SDRAM "
                "i/o width of Asymmetrical odd ranks"
             << std::endl;
    ...
    if ((tmp == SPD_JEDEC_DDR5_DIE_PER_PKG_RESERVED_BIT_001) ||
        (tmp > SPD_JEDEC_DDR5_DIE_PER_PKG_RESERVED_BITS))
    {
        cerr << "Bad data in vpd byte 8 i.e " << tmp
             << ". So Can't calculate Die Per "
                "Package of Asymmetrical odd ranks"
             << std::endl;
    ...
    {
        cerr << "Bad data in vpd byte 8 i.e " << tmp
             << ". So Can't calculate SDRAM Density "
                "Per Die of Asymmetrical odd ranks"
             << std::endl;
...
auto memoryVpdParser::getDDR5DimmSize(Binary::const_iterator& iterator)
{
    ...
    size_t numOfChanlPerDimm = 0, primaryBusWidthPerChanl = 1,
    ...
    {
        cerr << "Bad data in vpd byte 235 i.e " << tmp
             << ". So Can't calculate number of "
                "channel per DIMM"
             << std::endl;
    ...
    {
        cerr << "Bad data in vpd byte 235 i.e " << tmp
             << ". So Can't calculate primary bus "
                "width"
             << std::endl;
    ...
    if (!getSdramParamsFromSPDData(iterator, sdramIoWid, diePerPkg,
                                   sdramDnstyPerDie))
    {
    ...
    {
        dimmSize = (numOfChanlPerDimm * (primaryBusWidthPerChanl / sdramIoWid) *
    ...
    else
    {
        ...
        if (!getSdramParamsForAsymmetricModule(iterator, secSdramIoWid,
                                               secSdramDiePerPkg,
                                               secSdramDnstyPerDie))
        {
            // returning dimmSize as 0
        ...
        size_t numOfPkgRankPerChnlForPriSdRam = 0;
        size_t numOfPkgRankPerChnlForSecSdRam = 0;

        getEvenOddCount(numOfPkgRankPerChnl, numOfPkgRankPerChnlForPriSdRam,
                        numOfPkgRankPerChnlForSecSdRam);

        size_t capacityOfPrimSdRam =
            (numOfChanlPerDimm * (primaryBusWidthPerChanl / sdramIoWid) *
             diePerPkg * (sdramDnstyPerDie / 8) *
             numOfPkgRankPerChnlForPriSdRam);

        size_t capacityOfSecSdram =
            (numOfChanlPerDimm * (primaryBusWidthPerChanl / secSdramIoWid) *
...
kwdVpdMap memoryVpdParser::readKeywords(Binary::const_iterator iterator)
{
    ...
    auto dimmSize = getDDRXDimmSize(iterator);
    if (!dimmSize)
    {
