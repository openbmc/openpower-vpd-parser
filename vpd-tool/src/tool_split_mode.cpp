    ...
    const std::optional<std::string>& i_filePath) const noexcept
{
    try
    {
        ...
        if (!l_ubootResult || !(*l_ubootResult))
        {
            std::cerr
        ...
        std::cout
            << std::format(
                   ...
                   "Next steps:\n"
                   "  1. If any VPD record/keyword in the system VPD file needs to be updated,\n"
                   "     use the following command before rebooting:\n"
...
int SplitMode::exitSplitMode() const noexcept
{
    try
    {
        ...
        {
            std::cerr
                << "Failed to set and validate U-Boot variables for split mode. Aborting exit split mode."
                << std::endl;
            return static_cast<int>(l_ubootResult.error());
        }
        ...
        std::cout
            << "Exit split mode completed. Proceed with factory reset and boot the BMC with CDFP connected."
            << std::endl;
    }
    catch (const std::exception& l_ex)
    {
