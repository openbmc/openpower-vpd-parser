                 ...
                 const std::string& i_keywordValue)
{
    ...
    // Create optional record name based on whether record option was provided
    std::optional<std::string> l_recordName =
        (!i_recordOption->empty()) ? std::optional<std::string>(i_recordName) : std::nullopt;

    vpd::VpdTool l_vpdToolObj;
    return l_vpdToolObj.writeKeyword(i_vpdPath, l_recordName, i_keywordName,
                ...
                const std::string& i_keywordName, const std::string& i_filePath)
{
    ...
    vpd::VpdTool l_vpdToolObj;
    return l_vpdToolObj.readKeyword(i_vpdPath, l_recordName, i_keywordName,
