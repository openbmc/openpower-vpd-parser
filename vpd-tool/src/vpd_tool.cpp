    ...
    const std::string& i_fileToSave)
{
        ...
        if (i_onHardware)
        {
            ...
            else
            {
                ...
                l_keywordValue = utils::readKeywordFromHardware(
                    i_vpdPath, std::make_tuple(i_recordName.value(), i_keywordName));
            }
        }
        else
        {
            if (!i_recordName.has_value())
            {
                std::cerr << "Record name is required for DBus read operations."
                          << std::endl;
                return l_rc;
            }
            ...
            l_keywordValue = utils::readDbusProperty(
                ...
                constants::ipzVpdInfPrefix + i_recordName.value(), l_keywordName);
        }
    ...
    catch (const std::exception& l_ex)
    {
        ...
        std::cerr << "Read keyword's value failed for path: " << i_vpdPath
                  << ", Record: " << (i_recordName.has_value() ? i_recordName.value() : "") << ", Keyword: "
    ...
    const bool i_onHardware) noexcept
{
    ...
    catch (const std::exception& l_ex)
    {
        ...
        std::cerr << "Write keyword's value for path: " << i_vpdPath
                  << ", Record: " << (i_recordName.has_value() ? i_recordName.value() : "")
