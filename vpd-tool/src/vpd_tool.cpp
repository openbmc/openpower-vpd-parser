...
void VpdTool::populateInterfaceJson(const std::string& i_invPath,
                                    const std::string& i_infName,
                                    const std::vector<std::string>& i_propList,
                                    nlohmann::json& i_fruJsonObject) const
{
    nlohmann::json l_InterfaceJson = nlohmann::json::object({});

    auto l_readProperties = [i_invPath, &l_InterfaceJson, i_infName,
                             this](const std::string& i_property) {
        const nlohmann::json l_propertyJson =
            getInventoryPropertyJson<PropertyType>(i_invPath, i_infName,
                                                   i_property);
        l_InterfaceJson.insert(l_propertyJson.cbegin(), l_propertyJson.cend());
    };
    ...
    {
        i_fruJsonObject.insert(l_InterfaceJson.cbegin(),
                               l_InterfaceJson.cend());
    }
}

void VpdTool::populateFruJson(const std::string& i_invPath,
                              nlohmann::json& i_fruJsonObject,
                              std::vector<std::string> i_interfaceList) const
{
    for (const auto& l_interface : i_interfaceList)
    {
        if (l_interface == constants::inventoryItemInf)
        {
            ...
            populateInterfaceJson<std::string>(i_invPath,
                                               constants::inventoryItemInf,
                                               l_properties, i_fruJsonObject);
            continue;
        }
        ...
        if (l_interface == constants::viniInf)
        {
            ...
            populateInterfaceJson<vpd::types::BinaryVector>(
                i_invPath, constants::viniInf, l_properties, i_fruJsonObject);
            continue;
        }
        ...
        if (l_interface == constants::assetInf)
        {
            ...
            populateInterfaceJson<std::string>(i_invPath, constants::assetInf,
                                               l_properties, i_fruJsonObject);
            continue;
        }
        ...
        if (l_interface == constants::networkInf)
        {
            ...
            populateInterfaceJson<std::string>(i_invPath, constants::networkInf,
                                               l_properties, i_fruJsonObject);
            continue;
        }
        ...
        if (l_interface == constants::slotNumInf)
        {
            ...
            populateInterfaceJson<uint32_t>(i_invPath, constants::slotNumInf,
                                            l_properties, i_fruJsonObject);
            continue;
        }
        ...
        if (l_interface == constants::i2cDeviceInf)
        {
            ...
            populateInterfaceJson<uint32_t>(i_invPath, constants::i2cDeviceInf,
                                            l_properties, i_fruJsonObject);
            continue;
        }
    }
...
nlohmann::json VpdTool::getFruProperties(const std::string& i_objectPath) const
{
    ...
    // Don't process cables.
    if ((l_invPathLeafValue.find("cable") != std::string::npos))
    {
