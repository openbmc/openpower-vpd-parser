    ...
    const std::string& i_fileToSave) const noexcept
{
    ...
    catch (const std::exception& l_ex)
    {
        ...
        std::cerr << "Read keyword's value failed for path: " << i_vpdPath
                  << (!i_recordName.empty()
                          ? (std::format(", Record : {}", i_recordName))
                          : "")
...
int VpdTool::dumpObject(std::string i_fruPath) const noexcept
{
    ...
    try
    {
        ...
        if (!l_fruJson.value().empty())
        {
            ...
            const auto l_printJsonRes = utils::printJson(l_resultJsonArray);
    ...
    const bool i_onHardware) const noexcept
{
    ...
    try
    {
        ...
        else
        {
            ...
            // Check if record is part of the input inventory path and retrive
            // the base inventory path of the FRU, as vpd-manager service D-Bus
            // method "updateKeyword" accepts only the base inventory path to
            // update keyword.
            const auto l_isRecordPartOfFru =
                utils::getBaseInventoryPathForRecord(i_vpdPath, i_recordName);
            ...
            auto l_paramsToWrite = std::make_tuple(i_recordName, i_keywordName,
                                                   l_binaryVector.value());
            l_rc = utils::writeKeyword(l_isRecordPartOfFru.value(),
                                       l_paramsToWrite);
        }
                           ...
                           bool i_dumpTable) const noexcept
{
    ...
    try
    {
        ...
        else
        {
            ...
            const auto l_printJsonRes = utils::printJson(l_resultInJson);
    ...
    const std::string& i_fruPath) const noexcept
{
    ...
    try
    {
        if (utils::validateRedundantEeprom(i_fruPath))
        {
            std::println(
