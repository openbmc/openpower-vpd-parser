    ...
    const std::string& i_keywordName) const noexcept
{
    ...
    if (l_entry != m_biosAttributeVpdKeywordMap.end())
    {
        ...
        for (const auto& l_biosAttribute : l_biosAttributeList)
        {
            std::cout << "_SR " << l_biosAttribute << std::endl;
            // const auto l_attributeValue =
            // utils::biosGetAttributeMethodCall(l_biosAttribute);
        }
    }
    return l_result;
}
