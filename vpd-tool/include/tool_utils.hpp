 ...
 *
 * @return  On success, contains the captured output lines. Corresponding error code on failure.
 */
    ...
    const std::string& i_command) noexcept
{
    try
    {
        ...
        {
            std::cerr << std::format("Command [{}] exited with non-zero status: {}",
                       i_command, WEXITSTATUS(l_status)) << std::endl;
            return std::unexpected(ErrorCode::SYS_CMD_FAILED);
        }
    ...
    {
        std::cerr<< std::format("Exception while executing command {}: {}", i_command, l_ex.what());
        return std::unexpected(ErrorCode::STANDARD_EXCEPTION);
    }
}
} // namespace utils
