 ...
 * tuple of both current as well as pending value for that attribute. The
 * API return only the current attribute value if found. API returns an
 * empty variant of type BiosAttributeCurrentValue in case of any error.
 *
