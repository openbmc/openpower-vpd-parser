 ...
 */
inline std::expected<types::BinaryVector, int> convertToBinary(
    const std::string& i_value)
{
    if (i_value.empty())
    {
        ...
        return std::unexpected(
            static_cast<int>(ErrorCode::INVALID_INPUT_PARAMETER));
    }
 ...
 *
 */
inline std::expected<std::string, int> readValueFromFile(
    const std::string& i_filePath)
{
    ...
    if (!std::filesystem::exists(i_filePath, l_ec))
    {
        ...
        return std::unexpected(static_cast<int>(ErrorCode::FILE_SYSTEM_ERROR));
    }

    if (std::filesystem::is_empty(i_filePath, l_ec))
    {
        std::cerr << "File[" << i_filePath << "] is empty." << std::endl;
        return std::unexpected(static_cast<int>(ErrorCode::EMPTY_FILE));
    }
    else if (l_ec)
    {
