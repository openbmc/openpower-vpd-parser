 ...
 *
 * @return - Value read from Dbus. Corresponding error code is returned in
 * failure case.
 */
    ...
    const std::string& i_interface, const std::string& i_property)
{
    ...
    if (i_serviceName.empty() || i_objectPath.empty() || i_interface.empty() ||
        i_property.empty())
    {
        std::cout << "Empty input parameter received" << std::endl;
 ...
 * Corresponding error code is returned incase of exception.
 */
