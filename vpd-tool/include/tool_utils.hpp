    ...
    convertToBinary(const std::string& i_value)
{
    if (i_value.empty())
    {
        std::cerr << "Provide a valid hexadecimal input. (Ex. 0x30313233)"
                  << std::endl;
        return std::unexpected(error_codes::INVALID_INPUT_PARAMETER);
    }
    ...
    const std::string& i_filePath)
{
    ...
    if (!std::filesystem::exists(i_filePath, l_ec))
    {
        ...
        return std::unexpected(error_codes::FILE_SYSTEM_ERROR);
    }

    if (std::filesystem::is_empty(i_filePath, l_ec))
    {
        std::cerr << "File[" << i_filePath << "] is empty." << std::endl;
        return std::unexpected(error_codes::EMPTY_FILE);
    }
    else if (l_ec)
    {
