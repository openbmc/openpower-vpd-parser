...
/**
 * @brief Check record and return base inventory path of FRU
 *
 ...
 * or lookup.
 */
inline std::expected<std::string, ErrorCode>
    validateRecordAndGetBaseInventoryPath(
    ...
    try
    {
        ...
        for (const auto& l_fru : l_fruList.items())
        {
            ...
            const std::string& l_baseInvPath =
                l_inventoryItems.at(0).value("inventoryPath", "");
    ...
    const std::string& i_command) noexcept
{
    ...
    try
    {
        ...
        if (!WIFEXITED(l_status))
        {
            std::cerr
                << std::format(
                       "Command [{}] did not exit normally. Signaled: [{}], signal: [{}].",
                       i_command, WIFSIGNALED(l_status),
                       WIFSIGNALED(l_status) ? WTERMSIG(l_status) : 0)
                << std::endl;
    ...
    const std::string& i_ubootVariableValue) noexcept
{
    if (i_ubootVariable.empty())
    {
        std::cerr << "Received empty U-Boot variable name." << std::endl;
        return std::unexpected(ErrorCode::INVALID_INPUT_PARAMETER);
    }

    if (i_ubootVariableValue.empty())
    {
        std::cerr << "Received empty U-Boot variable value." << std::endl;
        return std::unexpected(ErrorCode::INVALID_INPUT_PARAMETER);
    }
    ...
    try
    {
        ...
        return (i_ubootVariableValue == l_readVarRes.value());
    }
    catch (const std::exception& l_ex)
    {
        std::cerr
