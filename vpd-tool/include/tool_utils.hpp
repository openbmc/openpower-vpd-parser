 ...
 *
 * The API reads the given attribuute from BIOS Config Manager and returns a
 * tuple of both current as well as pending value for that attribute. The API
 * return only the current attribute value if found. API returns an empty
 * variant of type BiosAttributeCurrentValue in case of any error.
 *
...
inline types::BiosAttributeCurrentValue
    biosGetAttributeMethodCall(const std::string& i_attributeName) noexcept
{
 ...
 *
 * @param [in] io_string - Input string.
 *
