...
respective Bitbake recipe which defines `SYSTEM_VPD_FILE_PATH` meson option.
...
prepending `/var/lib/vpd/file` to the System VPD EEPROM path mentioned in the
respective Bitbake recipe which defines `SYSTEM_VPD_FILE_PATH` meson option.

Example: If the `SYSTEM_VPD_FILE_PATH` meson option in respective Bitbake recipe
specifies System VPD EEPROM path as `/sys/bus/i2c/drivers/at24/8-0050/eeprom`,
vpd-manager will look for the file
`/var/lib/vpd/file/sys/bus/i2c/drivers/at24/8-0050/eeprom`
...
to refer to the respective system workbook or the `SYSTEM_VPD_FILE_PATH` meson
option specified in respective Bitbake recipe.

Example: If System VPD is on EEPROM path
`/sys/bus/i2c/drivers/at24/1-0051/eeprom`, the VPD image needs to be copied to
`/var/lib/vpd/file/sys/bus/i2c/drivers/at24/1-0051/eeprom`
