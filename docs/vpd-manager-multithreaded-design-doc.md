...
**Our Solution**: Each thread creates its own independent D-Bus connection that's
...
2. **Stack-Allocated**: The `bus` object is created on the stack within each
   function call
3. **Thread-Local Scope**: Connection exists only for the duration of the
   D-Bus call
4. **Synchronous Calls**: Each `bus.call()` is blocking and completes before
...
Each thread uses its own short-lived connection, which is consistent with the
sd-bus requirement that a connection not be shared across threads.
