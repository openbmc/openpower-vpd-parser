...
- **holdidlepath** -  The absolute sysfs path to the mux driver's hold_idle control node.
...
- **postAction** - Executed after successful FRU initialization or VPD collection.
- **postFailAction** - Executed when FRU initialization or VPD collection fails, allowing
the system to be restored to a safe state.
