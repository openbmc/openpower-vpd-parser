...
Specifies the device tree reference.
...
Multiple phases allowed.

##### `<ACTION_TAG>`

Specifies the action to perform in a given phase.
...
> ⚠️ **Important**
> Although the JSON structure is generic and allows future extensions, the current
> implementation recognizes only the action groups and action tags explicitly
> supported by the source code.
> Adding new action groups or action tags requires corresponding updates in the
> source code.
...
````json
{
   "muxes": "<Each mux must be defined with the below fields>" [
        {
