...
>i2bus :
The I2C bus number where the mux is connected.
>deviceaddress :
The 7-bit I2C address of the mux device.
>holdidlepath :
The absolute sysfs path to the mux driver's hold_idle control node.
...
preAction,postAction, postFailAction). Multiple groups are allowed.
Currently following action groups are supported:

#### "preAction"

Executed before FRU initialization or VPD collection.

#### "postAction"

Executed after successful FRU initialization or VPD collection.

#### "postFailAction"

Executed when FRU initialization or VPD collection fails, allowing the system
to be restored to a safe state.
...
>collection
The collection phase is executed when the FRU's VPD data is getting collected.
>deletion
The deletion phase is executed when a FRU is removed.
...
>gpioPresence
Checks GPIO pin to detect FRU's presence.
>setGpio
Set a GPIO pin to a required state for specific action.
>systemCmd
