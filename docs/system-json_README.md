...
It is the file path where essential FRU VPD records are backed up.This functionality
enables data synchronization between any two hardware entities to ensure recovery
when one is replaced.
Note: If both entities are replaced at the same time, data cannot be restored.
